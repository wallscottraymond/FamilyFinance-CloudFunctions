...
export const PURGE_STATUS_COLLECTION = "purge_status";
 ...
 */
export async function is_user_purging(user_id: string): Promise<boolean> {
  const doc = await getFirestore()
    .collection(PURGE_STATUS_COLLECTION)
    .doc(user_id)
    .get();
  if (!doc.exists) {
    return false;
  }
  const state = (doc.data() as PurgeStatus).state;
 ...
 */
export async function set_purge_status(
  user_id: string,
  patch: Partial<PurgeStatus>
): Promise<void> {
  await getFirestore()
    .collection(PURGE_STATUS_COLLECTION)
    .doc(user_id)
    .set(
      { user_id, updated_at: Timestamp.now(), ...patch },
      { merge: true }
    );
}
