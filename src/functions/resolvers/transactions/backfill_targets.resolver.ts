 ...
 */
export async function resolve_backfill_user_ids(
  ctx: TraceContext
): Promise<string[]> {
  return user_repo.get_all_ids(ctx);
}
 ...
 */
export async function resolve_user_backfill_targets(
  ctx: TraceContext,
  user_id: string
): Promise<UserBackfillTargets> {
  const [transaction_ids, budgets] = await Promise.all([
    transaction_repo.get_ids_by_user_id(ctx, user_id),
    budget_repo.get_by_user_id(ctx, user_id),
  ]);
  console.log(
    `[${ctx.trace_id}] resolve_user_backfill_targets: user=${user_id}, ` +
