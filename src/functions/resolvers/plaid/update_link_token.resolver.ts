 ...
 */
export async function resolve_update_link_token_dependencies(
  ctx: TraceContext,
  input: ResolveUpdateLinkTokenInput
): Promise<UpdateLinkTokenDependencies> {
  const span = create_span(ctx, "resolver", "resolve_update_link_token_dependencies");
  ...
  const plaid_item: UpdateLinkTokenDependencies["plaid_item"] = {
    id: item_doc.id,
    ...
    status: (item_data.status || "good") as PlaidItemStatus,
  ...
  if (user_owns_item) {
    try {
      ...
      console.warn(
        `[${ctx.trace_id}] Failed to fetch user profile for ${input.user_id}:`,
        error
      );
  ...
  try {
    ...
    console.warn(
      `[${ctx.trace_id}] Failed to count relink attempts:`,
      error
    );
