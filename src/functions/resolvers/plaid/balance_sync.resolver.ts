 ...
 */
export async function resolve_balance_sync_dependencies(
  ctx: TraceContext,
  input: ResolveBalanceSyncInput
): Promise<BalanceSyncDependencies> {
  ...
  const items: BalanceSyncDependencies["items"] = [];
  ...
  for (const doc of items_snapshot.docs) {
    ...
    if (!encrypted_token) {
      console.warn(
        `[${ctx.trace_id}] Plaid item ${doc.id} has no access token, skipping`
      );
      continue;
    }

    try {
      const access_token = decryptAccessToken(encrypted_token);
      items.push({
      ...
      console.error(
        `[${ctx.trace_id}] Failed to decrypt access token for item ${doc.id}:`,
        error
      );
    }
  }
  ...
  const accounts_by_item = new Map<string, Array<{
    id: string;
  ...
  console.log(
    `[${ctx.trace_id}] Resolved balance sync dependencies: ` +
