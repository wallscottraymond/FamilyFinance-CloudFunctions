 ...
 */
export const fix_account_plaid_ids = onCall(async () => {
  ...
  try {
    ...
    interface AccountToFix {
      id: string;
      account_id: string;
      name: string;
    }
    const missing_plaid_ids: AccountToFix[] = [];
    ...
    throw new HttpsError(
      "internal",
      `Failed to fix accounts: ${error instanceof Error ? error.message : String(error)}`
    );
  }
});
