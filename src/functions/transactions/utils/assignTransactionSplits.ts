...
import { validateAndFixBudgetIds } from './validateBudgetIds';
...
export async function assignTransactionSplits(
  ...
  try {
    ...
    const validatedSplits = await validateAndFixBudgetIds(userId, currentTransaction.splits);

    // Count how many budgetIds were fixed
    changes.budgetIdsFixed = currentTransaction.splits.filter((split, index) =>
      split.budgetId !== validatedSplits[index].budgetId
    ).length;
    ...
    const transactionAmount = currentTransaction.splits.reduce(
    ...
    const validationResult = validateAndRedistributeSplits(
      transactionAmount,
      currentTransaction.splits as any
    );
    ...
    const matchedTransactions = await matchTransactionSplitsToBudgets(
      [currentTransaction],
      userId
    );
    ...
    changes.budgetsReassigned = currentTransaction.splits.filter((split, index) =>
      split.budgetId !== matchedTransaction.splits[index].budgetId
    ).length;
