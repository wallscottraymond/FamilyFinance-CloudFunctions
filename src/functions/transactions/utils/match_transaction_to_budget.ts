 ...
 */
export async function match_transaction_to_budget(
  user_id: string,
  ...
  const transaction_timestamp = Timestamp.fromDate(transaction_date);
  ...
  const budgets_query = await queryDocuments('budgets', {
    where: [
  ...
  for (const budget of budgets_query) {
    const budget_data = budget as any;
    ...
    if (budget_start) {
      const start_timestamp = budget_start instanceof Timestamp
        ? budget_start
        : Timestamp.fromDate(new Date(budget_start));
      ...
      if (!is_ongoing && budget_end) {
        const end_timestamp = budget_end instanceof Timestamp
          ? budget_end
          : Timestamp.fromDate(new Date(budget_end));
