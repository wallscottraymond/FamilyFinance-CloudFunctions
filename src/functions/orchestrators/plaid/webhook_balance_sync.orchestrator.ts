 ...
 */
export async function webhook_balance_sync_orchestrator(
  ctx: OrchestratorContext<WebhookBalanceSyncInput>
): Promise<WebhookBalanceSyncResponse> {
  const start_time = Date.now();

  console.log(
    `[${ctx.trace_id}] Webhook balance sync started: ` +
  ...
  const deps = await resolve_webhook_balance_sync_dependencies(
  ...
  try {
    ...
    console.error(
      `[${ctx.trace_id}] Failed to fetch balances from Plaid:`,
      error_msg
    );
  ...
  if (plaid_result.accounts.length === 0) {
    ...
    await plaid_webhook_repo.record_processed(
      ...
      ctx.input,
      true,
      "No accounts returned from Plaid"
    );
  ...
  try {
    const upsert_result = await account_repo.upsert_from_plaid(
      ...
      deps.item.plaid_item_id,
      deps.item.user_id,
      { id: deps.item.institution_id, name: deps.item.institution_name },
      deps.item.group_id
    );
    ...
    console.log(
      `[${ctx.trace_id}] Upserted accounts: created=${accounts_created}, updated=${accounts_updated}`
    );

  } catch (error) {
    const error_msg = error instanceof Error ? error.message : "Unknown error";
    console.error(
      `[${ctx.trace_id}] Failed to upsert accounts:`,
      error_msg
    );
  ...
  for (const event_payload of events_to_emit) {
    try {
      ...
      console.error(
        `[${ctx.trace_id}] Failed to emit balance_updated event:`,
        error
      );
    }
  }
  ...
  await plaid_webhook_repo.record_processed(
    ...
    ctx.input,
    true,
    `Synced: ${accounts_created} created, ${accounts_updated} updated`
  );

  const duration_ms = Date.now() - start_time;
  console.log(
    `[${ctx.trace_id}] Webhook balance sync completed in ${duration_ms}ms: ` +
    `${accounts_created} created, ${accounts_updated} updated, ` +
    `${events_to_emit.length} balance changes`
  );

  // Check performance budget
  if (duration_ms > WEBHOOK_BALANCE_SYNC_BUDGET.max_time_ms) {
    console.warn(
      `[${ctx.trace_id}] Webhook balance sync exceeded time budget: ` +
      `${duration_ms}ms > ${WEBHOOK_BALANCE_SYNC_BUDGET.max_time_ms}ms`
    );
  }
