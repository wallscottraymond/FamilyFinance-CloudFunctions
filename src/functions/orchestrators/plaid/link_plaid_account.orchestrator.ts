 ...
 */
export async function link_plaid_account_orchestrator(
  ctx: OrchestratorContext<LinkPlaidAccountInput>
): Promise<LinkPlaidAccountOrchestratorResult> {
  const span = create_span(ctx, "orchestrator", "link_plaid_account");
  ...
  try {
    ...
    if (idempotency_result.is_duplicate) {
      if (idempotency_result.status === "completed") {
        ...
        return idempotency_result.cached_result as LinkPlaidAccountOrchestratorResult;
      }
    ...
    // =========================================================================
    const deps = await resolve_link_account_dependencies(ctx, {
      user_id: ctx.user_id,
    ...
    if (deps.institution_already_linked) {
      console.log(
        `[${ctx.trace_id}] Institution ${ctx.input.institution_id} already linked ` +
    ...
    // 3. DOMAIN SERVICE: Validate request
    // =========================================================================
    const validation = validate_link_account_request({
      user_id: ctx.user_id,
    ...
    // =========================================================================
    const raw_response = await exchange_public_token(ctx.input.public_token);
    const exchange_result = transform_token_exchange_response(raw_response);
    perf.reads++; // External API call counted as read

    // =========================================================================
    // 5. DOMAIN SERVICE: Validate Plaid item for creation
    // =========================================================================
    const item_validation = validate_plaid_item_for_creation({
    ...
    if (item_validation.validation_errors && item_validation.validation_errors.length > 0) {
      await fail_key(ctx, ctx.idempotency_key, "Item validation failed");
      return {
        success: false,
    ...
    await plaid_item_repo.save(ctx, plaid_item);
    perf.writes++;

    // =========================================================================
    // 7. EMIT EVENTS
    // =========================================================================
    const event_payload: PlaidItemCreatedPayload = {
    ...
    events.emit(PLAID_EVENTS.ITEM_CREATED, event_payload);

    // =========================================================================
    // 8. COMPLETE IDEMPOTENCY KEY
    // =========================================================================
    const result: LinkPlaidAccountOrchestratorResult = {
      success: true,
      data: {
        item_id: exchange_result.item_id,
        institution_id: ctx.input.institution_id,
    ...
    if (is_budget_exceeded(perf, LINK_PLAID_ACCOUNT_BUDGET)) {
      console.warn(
        `[${ctx.trace_id}] Performance budget exceeded for link_plaid_account: ` +
    ...
    log_operation_error(
      ...
      { user_id: ctx.user_id, error_code: "LINK_PLAID_ACCOUNT_FAILED" }
    );
