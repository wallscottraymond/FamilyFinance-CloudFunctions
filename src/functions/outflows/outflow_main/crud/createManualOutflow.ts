 ...
 * Create Manual Outflow Cloud Function
 */
export const createManualOutflow = onCall<CreateRecurringOutflowRequest, Promise<CreateRecurringOutflowResponse>>(
  async (request) => {
    try {
      ...
      const outflowDoc = {
        ...
        access: buildAccessControl(userId, userId, groupId ? [groupId] : []),
      ...
      console.log(`[createManualOutflow] Created manual outflow ${outflowRef.id} for user ${userId}`);
      ...
      console.error('[createManualOutflow] Error:', error);
