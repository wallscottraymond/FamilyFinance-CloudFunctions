...
import { Budget, BudgetPeriodDocument, PeriodType, SourcePeriod, PrimePeriodContribution } from '../../../types';
 ...
 */
export function findOverlappingPrimePeriods(
  targetStart: Date,
  targetEnd: Date,
  sortedPrimePeriods: BudgetPeriodDocument[]
): BudgetPeriodDocument[] {
  const overlapping: BudgetPeriodDocument[] = [];
 ...
 */
export function calculatePrimeContributions(
  targetStart: Date,
  targetEnd: Date,
  overlappingPrimes: BudgetPeriodDocument[]
): { totalAmount: number; breakdown: PrimePeriodContribution[] } {
  ...
  const contributionMap: Map<string, {
    prime: BudgetPeriodDocument;
  ...
  const breakdown: PrimePeriodContribution[] = [];
  ...
  for (const [primeId, contribution] of contributionMap) {
    breakdown.push({
      ...
      overlapStart: Timestamp.fromDate(contribution.overlapStart!),
      overlapEnd: Timestamp.fromDate(contribution.overlapEnd!),
    });
  }
 ...
 */
export async function generateNonPrimeBudgetPeriods(
  db: admin.firestore.Firestore,
  budgetId: string,
  budget: Budget,
  primePeriods: BudgetPeriodDocument[],
  startDate: Date,
  endDate: Date,
  ...
  const sortedPrimePeriods = [...primePeriods].sort((a, b) =>
  ...
  const nonPrimeBudgetPeriods: BudgetPeriodDocument[] = [];
