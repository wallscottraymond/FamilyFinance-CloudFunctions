 ...
 */
export async function updateUserPeriodSummary(
  userId: string,
  periodType: string,
  ...
  console.log(
    `[updateUserPeriodSummary] Starting update for user: ${userId}, period: ${sourcePeriodId}, type: ${periodType}`
  );
  ...
  try {
    ...
    if (existingSummary.exists) {
      ...
      console.log(
        `[updateUserPeriodSummary] Updating existing summary: ${summaryId}`
      );
      ...
      console.log(
        `[updateUserPeriodSummary] Successfully updated summary: ${summaryId}`
      );
    } else {
      // Create new summary
      console.log(
        `[updateUserPeriodSummary] Creating new summary: ${summaryId}`
      );

      await summaryRef.set(summary);

      console.log(
        `[updateUserPeriodSummary] Successfully created summary: ${summaryId}`
      );
    }
    ...
    console.error(
      `[updateUserPeriodSummary] Error updating summary for user ${userId}, period ${sourcePeriodId}:`,
      error
    );
    throw new Error(
      `Failed to update user period summary: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
