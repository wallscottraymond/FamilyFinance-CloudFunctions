...
export async function fetchBudgetsBatch(
  ...
  console.log(
    `[fetchBudgetsBatch] Fetching budget periods for user: ${userId}, period: ${sourcePeriodId}, type: ${periodType}`
  );
  ...
  try {
    ...
    console.log(
      `[fetchBudgetsBatch] Found ${budgetPeriods.length} budget periods for ${periodType}`
    );
    ...
    throw new Error(
      `Failed to fetch budget periods for user ${userId} in period ${sourcePeriodId} (${periodType}): ${error}`
    );
  }
}
