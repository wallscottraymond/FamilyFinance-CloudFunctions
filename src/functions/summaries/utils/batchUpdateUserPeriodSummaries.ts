 ...
 */
export async function batchUpdateUserPeriodSummaries(
  userId: string,
  periodIds: string[],
  periodType: string
): Promise<number> {
  console.log(
    `[batchUpdateUserPeriodSummaries] Batch updating ${periodIds.length} summaries for user: ${userId}`
  );
  ...
  console.log(
    `[batchUpdateUserPeriodSummaries] Unique periods to update: ${uniquePeriodIds.length}`
  );

  // Update all summaries in parallel
  const updatePromises = uniquePeriodIds.map((sourcePeriodId) =>
    updateUserPeriodSummary(userId, periodType, sourcePeriodId, false).catch(
      (error) => {
        console.error(
          `[batchUpdateUserPeriodSummaries] Error updating summary for period ${sourcePeriodId}:`,
          error
        );
  ...
  console.log(
    `[batchUpdateUserPeriodSummaries] Completed in ${duration}ms`,
    {
 ...
 * @returns Count of summaries updated
 */
export async function batchUpdateUserPeriodSummariesFromOutflowPeriods(
  userId: string,
  outflowPeriodIds: string[]
): Promise<number> {
  console.log(
    `[batchUpdateUserPeriodSummariesFromOutflowPeriods] Fetching ${outflowPeriodIds.length} outflow periods`
  );

  // Fetch all outflow period documents
  const periodDocs = await Promise.all(
    outflowPeriodIds.map((id) =>
      db.collection("outflow_periods").doc(id).get()
    )
  );

  // Group by period type
  const periodsByType = new Map<string, Set<string>>();

  for (const doc of periodDocs) {
    if (!doc.exists) continue;

    const period = doc.data() as OutflowPeriod;
  ...
  console.log(
    `[batchUpdateUserPeriodSummariesFromOutflowPeriods] Grouped into ${periodsByType.size} period types`
  );
  ...
  for (const [periodType, sourcePeriodIds] of periodsByType.entries()) {
    console.log(
      `[batchUpdateUserPeriodSummariesFromOutflowPeriods] Updating ${sourcePeriodIds.size} ${periodType} summaries`
    );

    updatePromises.push(
      batchUpdateUserPeriodSummaries(
        userId,
        Array.from(sourcePeriodIds),
        periodType
      )
    );
  }
  ...
  console.log(
    `[batchUpdateUserPeriodSummariesFromOutflowPeriods] Total summaries updated: ${totalUpdated}`
  );
