 ...
 */
function calculateDaysUntil(date: Timestamp | undefined): number {
  if (!date) return 0;
...
function determineConfidenceLevel(
  ...
  hasPlaidPrediction: boolean
): 'high' | 'medium' | 'low' {
  if (isRegularSalary && hasPlaidPrediction) return 'high';
  if (isRegularSalary) return 'high';
  if (hasPlaidPrediction) return 'medium';
  return 'low';
}
...
export function calculateInflowSummary(
  ...
  const entries: InflowEntry[] = inflowPeriods.map(inflowPeriod => {
    ...
    const isRegularSalary =
      (inflowPeriod as any).isRegularSalary === true ||
