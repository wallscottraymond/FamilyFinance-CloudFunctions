 ...
 */
export const relink_attempt_repo = {
  /**
   ...
   */
  async create(ctx: TraceContext, input: RelinkAttemptInput): Promise<string> {
    const db = get_db();
    const doc = db.collection(COLLECTION).doc();

    const attempt: Omit<RelinkAttempt, "id"> & { id: string } = {
      id: doc.id,
      ...input,
    ...
    console.log(
      `[${ctx.trace_id}] Created relink attempt ${doc.id} for item ${input.item_id}`
    );
  ...
  async mark_completed(
    ...
    console.log(
      `[${ctx.trace_id}] Marked relink attempt ${attempt_id} as ${success ? "successful" : "failed"}`
    );
  },
   ...
   */
  async mark_all_successful_for_item(
    ctx: TraceContext,
    item_id: string
  ): Promise<number> {
    const db = get_db();
    ...
    console.log(
      `[${ctx.trace_id}] Marked ${pending_snapshot.size} relink attempts as successful for item ${item_id}`
    );
  ...
  async count_recent(
    ...
    const window_start = Timestamp.fromDate(
      new Date(Date.now() - RELINK_ATTEMPT_WINDOW_HOURS * 60 * 60 * 1000)
    );
   ...
   */
  async get_most_recent(
    ctx: TraceContext,
    item_id: string
  ): Promise<RelinkAttempt | null> {
    const db = get_db();
    ...
    return snapshot.docs[0].data() as RelinkAttempt;
  },
  ...
  async cleanup_old_attempts(
    ...
    console.log(
      `[${ctx.trace_id}] Cleaned up ${old_snapshot.size} old relink attempts`
    );
