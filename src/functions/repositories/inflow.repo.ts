...
export interface Inflow {
  ...
  // read from `removal_intervals`; `removed_by_user` is a denorm. Both preserved
  // across Plaid re-sync.
...
/** Stored (camelCase) shape of a removal interval on the inflow doc. */
...
export const inflow_repo = {
  ...
  async apply_period_suppression(
    ...
    console.log(
      `[${ctx.trace_id}] inflow_repo.apply_period_suppression ${id}: flipped ${flipped}/${snap.size} periods (user ${user_id})`
    );
    return flipped;
  },
  ...
  async hard_delete(ctx: TraceContext, id: string, user_id: string): Promise<WriteResult> {
    ...
    return create_write_result("inflow", id, "replace", before, null);
  },
  ...
  async mark_hidden(
    ...
    for (const chunk of chunk_for_batch(stream_ids)) {
      ...
      for (const id of chunk) {
        ...
        // eslint-disable-next-line @typescript-eslint/naming-convention
        const after: LegacyInflowDoc = { ...before, isHidden: hidden, updatedAt: now };
        batch.set(doc_ref(id), after);
        results.push(create_write_result("inflow", id, "replace", before, after));
        record_audit_entry_async({
          user_id,
