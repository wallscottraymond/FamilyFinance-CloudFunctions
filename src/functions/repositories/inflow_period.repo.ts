...
export const inflow_period_repo = {
   ...
   */
  async set_hidden_by_inflow_ids(
    ctx: TraceContext,
    inflow_ids: string[],
    hidden: boolean
  ): Promise<number> {
    ...
    console.log(
      `[${ctx.trace_id}] inflow_period.set_hidden_by_inflow_ids: ` +
  ...
  async set_active_by_account_id(
    ...
    console.log(
      `[${ctx.trace_id}] inflow_period.set_active_by_account_id: ` +
  ...
  async update_reconciliation(
    ...
    for (const chunk of chunk_for_batch(results)) {
      ...
      for (const r of chunk) {
        ...
        write_results.push(
          create_write_result("inflow_period", r.period_id, "merge", { id: r.period_id }, update_data)
        );
      }
      await batch.commit();
    }
  ...
  async update_occurrence_fields(
    ...
    for (const chunk of chunk_for_batch(entities)) {
      ...
      for (const e of chunk) {
        ...
        write_results.push(
          create_write_result("inflow_period", e.id, "merge", { id: e.id }, update_data)
        );
      }
      await batch.commit();
    }
