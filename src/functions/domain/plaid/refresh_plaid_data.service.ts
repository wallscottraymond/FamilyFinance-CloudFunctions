 ...
 */
export function aggregate_refresh_results(
  balance_result: BalanceSyncPhaseResult,
  transaction_results: TransactionSyncPhaseResult[],
  rate_limited_items: string[] = []
): RefreshPlaidDataResponse {
  // Aggregate transaction stats
  const transactions_added = transaction_results.reduce(
    (sum, r) => sum + r.added_count,
    0
  );
  const transactions_modified = transaction_results.reduce(
    (sum, r) => sum + r.modified_count,
    0
  );
  const transactions_removed = transaction_results.reduce(
    (sum, r) => sum + r.removed_count,
    0
  );
  const pending_migrated = transaction_results.reduce(
    (sum, r) => sum + r.pending_migrated_count,
    0
  );
 ...
 */
export function create_refresh_failure_response(
 ...
 */
export function create_balance_phase_result(
 ...
 */
export function create_transaction_phase_result(
  item_id: string,
  result: {
    success: boolean;
    added_count: number;
 ...
 */
export function validate_refresh_request(
  items_count: number,
