 ...
 */
export function estimate_slot_amounts(
  occurrence_days: number[],
  deposits: DepositForSlot[],
  recent_n = 6
): Map<number, number> {
  const slots = [...new Set(occurrence_days)];
  const out = new Map<number, number>();
  // Single (or no) occurrence day → nothing to differentiate; use the stream average.
  if (slots.length <= 1) return out;

  const by_slot = new Map<number, DepositForSlot[]>();
  for (const day of slots) by_slot.set(day, []);
