...
export function compute_period_reconciliation(
  ...
  const occ_sum = new Array<number>(due.length).fill(0); // combined (posted+pending)
  const occ_pending_sum = new Array<number>(due.length).fill(0);
  const occ_txn = new Array<string | null>(due.length).fill(null);
  const occ_pending_txn = new Array<string | null>(due.length).fill(null);
  ...
  const occurrences: OccurrenceStatus[] = due.map((d, i) => {
    ...
    return {
      due_date_ms: d,
      paid,
      transaction_id: occ_txn[i] ?? occ_pending_txn[i],
