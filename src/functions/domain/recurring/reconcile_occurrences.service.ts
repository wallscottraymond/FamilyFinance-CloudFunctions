...
/**
 * A Plaid pending deposit and its later POSTED version — or a sandbox recycled-id
 * duplicate — arrive as two transactions with the SAME amount a day or two apart. Both
 * get linked to the inflow stream, so income reconciliation would match one to its
 * expected slot and surface the OTHER as a phantom second "received" occurrence (and
 * double the actual). Two genuinely distinct deposits of the same amount are ≥ a pay
 * cycle apart, so this tight window never collapses real occurrences.
 */
const DUP_DEPOSIT_DAYS = 3;

/**
 * Collapse deposits that represent the SAME real payment (pending+posted pair / recycled
 * dup): same amount within `DUP_DEPOSIT_DAYS`. Keeps the earliest (deterministic); the
 * amount — all that drives the received figure — is identical either way. PURE.
 */
function dedupe_deposits(payments: ActualPayment[]): ActualPayment[] {
  const window_ms = DUP_DEPOSIT_DAYS * MS_PER_DAY;
  const sorted = [...payments].sort((a, b) => a.date_ms - b.date_ms);
  const kept: ActualPayment[] = [];
  for (const p of sorted) {
    const is_dup = kept.some(
      (k) =>
        Math.round(k.amount * 100) === Math.round(p.amount * 100) &&
        Math.abs(k.date_ms - p.date_ms) <= window_ms
    );
    if (!is_dup) kept.push(p);
  }
  return kept;
}
 ...
 * PURE FUNCTION.
 */
export function reconcile_occurrences(
  ...
  const reconciled: ReconciledOccurrence[] = expected.map((e) => ({
...
export function reconcile_income_occurrences(
  ...
  // Collapse pending+posted / duplicate deposits FIRST, so a single real paycheck can't
  // become two occurrences (one matched to its slot, one surfaced as a phantom extra).
  const ordered = dedupe_deposits(payments);
