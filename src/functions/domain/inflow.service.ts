 ...
 */
export function validate_inflows_for_sync(
  inflows: InflowForPersistence[]
): DomainResult<InflowForPersistence> {
  const valid_inflows: InflowForPersistence[] = [];
  ...
  for (const inflow of inflows) {
    ...
    if (errors.length === 0) {
      ...
      validation_errors.push(
        `Inflow ${inflow.id}: ${errors.join(", ")}`
      );
    }
  }

  if (validation_errors.length > 0) {
    return partial_success(valid_inflows, validation_errors);
  }

  return success_many(valid_inflows);
}
 ...
 * PURE FUNCTION.
 */
function validate_single_inflow(inflow: InflowForPersistence): string[] {
  const errors: string[] = [];
  ...
  if (!valid_frequencies.includes(inflow.frequency as AppFrequency)) {
 ...
 */
export function detect_duplicate_inflows(
  new_inflows: InflowForPersistence[],
  existing_inflows: ExistingInflowData[]
  ...
  for (const new_inflow of new_inflows) {
    ...
    const exact_match = existing_inflows.find(
      (e) => e.plaid_stream_id === new_inflow.plaid_stream_id
    );
 ...
 * PURE FUNCTION.
 */
function find_similar_inflow(
  new_inflow: InflowForPersistence,
  existing_inflows: ExistingInflowData[]
): { existing: ExistingInflowData; confidence: "high" | "medium" | "low"; reason: string } | null {
 ...
 * PURE FUNCTION.
 */
function calculate_inflow_match(
  new_inflow: InflowForPersistence,
  existing: ExistingInflowData
  ...
  const name_match = names_match(
    new_inflow.payer_name || new_inflow.description,
    existing.payer_name || existing.description
  );
  if (name_match === "exact") {
    score += 3;
 ...
 */
export function compute_inflow_merge_suggestions(
  plaid_inflows: InflowForPersistence[],
  manual_inflows: ExistingInflowData[]
): DomainResult<InflowMergeSuggestion> {
  const suggestions: InflowMergeSuggestion[] = [];

  // Only consider active manual inflows
  const active_manual = manual_inflows.filter(
 ...
 */
export function filter_inflows_needing_review(
  inflows: InflowForPersistence[]
): InflowForPersistence[] {
 ...
 */
export function compute_inflow_status(
