 ...
 */
export function compute_create_budget(
  ...
  if (dependencies.existing_budget_count >= MAX_BUDGETS_PER_USER) {
    validation_errors.push(
      `budget limit reached (${MAX_BUDGETS_PER_USER} budgets per user)`
    );
  }
  ...
  let budget_end: Date | null = null;
  if (input.is_ongoing === false) {
    budget_end = input.budget_end_date ? parse_date(input.budget_end_date) : null;
    if (!budget_end) {
      validation_errors.push(
        "budget_end_date is required and must be valid when is_ongoing is false"
      );
  ...
  const start_date = start as Date;
  const legacy_end = input.end_date
    ? parse_date(input.end_date) ?? compute_legacy_end_date(start_date, input.period)
  ...
  const entity: BudgetEntity = {
    ...
    end_date: Timestamp.fromDate(legacy_end),
    spent: 0,
    ...
    budget_end_date: budget_end ? Timestamp.fromDate(budget_end) : undefined,
 ...
 * PURE helper.
 */
function compute_legacy_end_date(start: Date, period: BudgetPeriodType): Date {
