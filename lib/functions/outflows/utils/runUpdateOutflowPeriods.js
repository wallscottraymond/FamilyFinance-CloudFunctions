...
async function runUpdateOutflowPeriods(db, outflowId, outflowBefore, outflowAfter) {
    ...
    try {
        ...
        // Step 3: Filter periods to update (only unpaid periods - preserve paid periods)
        const periodsToUpdate = [];
        let skippedCount = 0;
        for (const periodDoc of periodsSnapshot.docs) {
            const period = periodDoc.data();
            // Skip fully paid periods (preserve historical payment data)
            if (period.isPaid || period.isFullyPaid) {
                console.log(`[runUpdateOutflowPeriods] Skipping period ${periodDoc.id} - fully paid`);
                skippedCount++;
                continue;
            }
            // Skip partially paid periods (avoid confusion with changing amounts)
            if (period.isPartiallyPaid) {
                console.log(`[runUpdateOutflowPeriods] Skipping period ${periodDoc.id} - partially paid`);
                skippedCount++;
                continue;
            }
            // Update all other periods (future, current unpaid, past unpaid)
            periodsToUpdate.push(periodDoc);
        }
        result.periodsSkipped = skippedCount;
        console.log(`[runUpdateOutflowPeriods] Periods to update: ${periodsToUpdate.length}`);
        console.log(`[runUpdateOutflowPeriods] Periods skipped (paid): ${skippedCount}`);
        if (periodsToUpdate.length === 0) {
            console.log(`[runUpdateOutflowPeriods] No periods need updating (all are paid)`);
            result.success = true;
            return result;
        }
