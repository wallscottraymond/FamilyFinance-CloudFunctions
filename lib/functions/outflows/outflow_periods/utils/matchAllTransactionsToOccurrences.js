...
async function matchAllTransactionsToOccurrences(db, periodId, periodData) {
    try {
        ...
        const updatedOccurrences = periodData.occurrences
        ...
        for (const split of splits) {
            ...
            if (occurrenceIndex !== null) {
                ...
                updatedOccurrences[occurrenceIndex] = Object.assign(Object.assign({}, updatedOccurrences[occurrenceIndex]), { isPaid: true, transactionId: split.transactionId, transactionSplitId: split.splitId, paymentDate: split.transactionDate, amountPaid: split.amount, paymentType: split.paymentType || 'regular', isAutoMatched: split.isAutoMatched || false, matchedAt: split.matchedAt || firestore_1.Timestamp.now(), matchedBy: split.matchedBy || 'system' });
