 ...
 * ```
 */
function calculateAllOccurrencesInPeriod(outflow, sourcePeriod) {
    const periodStart = sourcePeriod.startDate.toDate();
    const periodEnd = sourcePeriod.endDate.toDate();
    console.log(`[calculateAllOccurrencesInPeriod] Calculating for ${outflow.description || 'Unnamed'} ` +
    ...
    while (currentOccurrence <= periodEnd) {
        ...
        const dueDate = firestore_1.Timestamp.fromDate(currentOccurrence);
        const drawDate = firestore_1.Timestamp.fromDate(adjustForWeekend(currentOccurrence));
        ...
        console.log(`[calculateAllOccurrencesInPeriod] Occurrence #${occurrenceIndex + 1}: ` +
            `ID=${occurrence.id}, ` +
    ...
    console.log(`[calculateAllOccurrencesInPeriod] Result: ${result.numberOfOccurrences} occurrence(s) found for ` +
