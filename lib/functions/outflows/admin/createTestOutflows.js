...
exports.createTestOutflows = void 0;
...
const formatRecurringOutflows_1 = require("../utils/formatRecurringOutflows");
const enhanceRecurringOutflows_1 = require("../utils/enhanceRecurringOutflows");
const batchCreateRecurringStreams_1 = require("../utils/batchCreateRecurringStreams");
...
exports.createTestOutflows = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 120,
}, async (request) => {
    ...
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated to create test outflows');
    }
    try {
        console.log('');
        console.log('🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪🧪');
        ...
        for (const streamId of testStreamIds) {
            try {
                const outflowDoc = await index_1.db.collection('outflows').doc(streamId).get();
        ...
        const inflowDoc = await index_1.db.collection('inflows').doc('stream_payroll_001').get();
        if (inflowDoc.exists) {
        ...
        const periodsSnapshot = await index_1.db.collection('outflow_periods')
        ...
        const plaidItemRef = index_1.db.collection('plaid_items').doc();
        ...
        if (rawInflowStreams.length > 0) {
            ...
            try {
                ...
                const formattedInflows = await (0, formatRecurringInflows_1.formatRecurringInflows)(rawInflowStreams, testPlaidItemId, targetUserId, testFamilyId);
                ...
                const enhancedInflows = await (0, enhanceRecurringInflows_1.enhanceRecurringInflows)(formattedInflows, targetUserId);
                ...
                const inflowResult = await (0, batchCreateRecurringStreams_1.batchCreateInflowStreams)(enhancedInflows, targetUserId);
        ...
        if (rawOutflowStreams.length > 0) {
            ...
            try {
                ...
                const formattedOutflows = await (0, formatRecurringOutflows_1.formatRecurringOutflows)(rawOutflowStreams, testPlaidItemId, targetUserId, testFamilyId);
                ...
                const enhancedOutflows = await (0, enhanceRecurringOutflows_1.enhanceRecurringOutflows)(formattedOutflows, targetUserId);
                ...
                console.log('');
                const outflowResult = await (0, batchCreateRecurringStreams_1.batchCreateOutflowStreams)(enhancedOutflows, targetUserId);
        ...
        console.error(error);
        console.error('');
        throw new https_1.HttpsError('internal', error instanceof Error ? error.message : 'Unknown error occurred while creating test outflows');
    }
});
//# sourceMappingURL=createTestOutflows.js.map