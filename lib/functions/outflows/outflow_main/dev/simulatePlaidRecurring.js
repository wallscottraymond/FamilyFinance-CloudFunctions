...
exports.simulatePlaidRecurring = void 0;
 ...
 */
function getMockPlaidData(permutation) {
    switch (permutation) {
        case 'standard':
        default:
...
exports.simulatePlaidRecurring = (0, https_1.onRequest)({
    cors: true,
    memory: '512MiB',
    timeoutSeconds: 60,
}, async (req, res) => {
    try {
        ...
        const userId = req.query.userId || 'IKzBkwEZb6MdJkdDVnVyTFAFj5i1';
        const groupId = req.query.groupId || 'test-family-1';
        const permutation = req.query.permutation || 'standard';
        ...
        if (rawInflowStreams.length > 0) {
            ...
            try {
                ...
                const formattedInflows = await (0, formatRecurringInflows_1.formatRecurringInflows)(rawInflowStreams, // Use 'as any' for mock Plaid data
                mockPlaidItemId, userId, groupId);
                ...
                const inflowResult = await (0, utils_1.batchCreateInflowStreams)(enhancedInflows, userId);
        ...
        if (rawOutflowStreams.length > 0) {
            ...
            try {
                ...
                const formattedOutflows = await (0, utils_1.formatRecurringOutflows)(rawOutflowStreams, // Use 'as any' for mock Plaid data
                mockPlaidItemId, userId, groupId);
                console.log(`✅ Step 1/3: Formatted ${formattedOutflows.length} outflow streams`);
                // Step 2: Enhance outflow streams (business logic)
                const enhancedOutflows = await (0, utils_1.enhanceRecurringOutflows)(formattedOutflows, userId);
                console.log(`✅ Step 2/3: Enhanced ${enhancedOutflows.length} outflow streams`);
                // Step 3: Batch create/update outflow streams
                const outflowResult = await (0, utils_1.batchCreateOutflowStreams)(enhancedOutflows, userId);
        ...
        // Return response
        res.status(200).json(Object.assign({ success: true, permutation,
            userId,
            groupId }, result));
    }
    catch (error) {
...
//# sourceMappingURL=simulatePlaidRecurring.js.map