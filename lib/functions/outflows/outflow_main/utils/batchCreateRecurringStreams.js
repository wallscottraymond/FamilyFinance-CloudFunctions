...
exports.batchCreateInflowStreams = batchCreateInflowStreams;
exports.batchCreateOutflowStreams = batchCreateOutflowStreams;
 ...
 */
async function batchCreateInflowStreams(inflows, userId) {
    ...
    try {
        for (const inflow of inflows) {
            try {
                ...
                const docRef = index_1.db.collection('inflows').doc(streamId);
                ...
                if (docSnapshot.exists) {
                    ...
                    await docRef.set(Object.assign(Object.assign({}, inflow), { createdAt: firestore_1.Timestamp.now(), updatedAt: firestore_1.Timestamp.now() }));
                    result.created++;
 ...
 */
async function batchCreateOutflowStreams(outflows, userId) {
    ...
    try {
        for (const outflow of outflows) {
            try {
                ...
                const docRef = index_1.db.collection('outflows').doc(streamId);
                const docSnapshot = await docRef.get();
                if (docSnapshot.exists) {
                    // Update existing stream (FLAT structure - no nested objects)
                    await docRef.update({
                    ...
                    // Create new stream with stream_id as document ID (FLAT structure)
                    await docRef.set(Object.assign(Object.assign({}, outflow), { createdAt: firestore_1.Timestamp.now(), updatedAt: firestore_1.Timestamp.now() }));
                    result.created++;
...
//# sourceMappingURL=batchCreateRecurringStreams.js.map