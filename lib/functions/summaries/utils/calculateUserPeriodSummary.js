...
async function calculateUserPeriodSummary(userId, periodType, sourcePeriodId, includeEntries = false) {
    ...
    const hiddenOutflowIds = new Set(outflowDefsSnap.docs
        .filter((d) => d.data().isHidden === true)
        .map((d) => d.id));
    const hiddenInflowIds = new Set(inflowDefsSnap.docs
        .filter((d) => d.data().isHidden === true)
        .map((d) => d.id));
    const visibleOutflowPeriods = outflowPeriods.filter((p) => { var _a; return !hiddenOutflowIds.has((_a = p.outflowId) !== null && _a !== void 0 ? _a : ""); });
    const visibleInflowPeriods = inflowPeriods.filter((p) => { var _a; return !hiddenInflowIds.has((_a = p.inflowId) !== null && _a !== void 0 ? _a : ""); });
    // Step 3: Convert resource periods to entry arrays
    const outflows = (0, calculateOutflowSummary_1.calculateOutflowSummary)(visibleOutflowPeriods);
    const budgets = (0, calculateBudgetSummary_1.calculateBudgetSummary)(budgetPeriods);
    const inflows = (0, calculateInflowSummary_1.calculateInflowSummary)(visibleInflowPeriods);
