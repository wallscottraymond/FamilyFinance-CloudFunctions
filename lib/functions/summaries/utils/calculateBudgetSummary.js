 ...
 */
function calculateBudgetSummary(budgetPeriods, includeEntries = true // ALWAYS include entries for tile rendering
) {
    var _a, _b, _c;
    console.log(`[calculateBudgetSummary] Calculating summary for ${budgetPeriods.length} budget periods`);
    // Initialize totals
    ...
    for (const budgetPeriod of budgetPeriods) {
        ...
        if (includeEntries) {
            ...
            const progressPercentage = allocatedAmount > 0
