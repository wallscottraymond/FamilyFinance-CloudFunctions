        ...
        try {
            ...
            (0, cors_1.corsMiddleware)(req, res, async () => {
                try {
                    ...
                    await index_1.db.collection('users').doc(user.uid).collection('plaidItems').doc(mockItemId).set({
                        itemId: mockItemId,
                        userId: user.uid,
                    ...
                    for (const account of mockAccounts) {
                        await index_1.db.collection('users').doc(user.uid).collection('plaidAccounts').doc(account.id).set({
 ...
 */
function generateMockBalance(accountType, accountSubtype) {
    const random = Math.random();
    switch (accountType.toLowerCase()) {
        case 'depository':
            switch (accountSubtype === null || accountSubtype === void 0 ? void 0 : accountSubtype.toLowerCase()) {
                case 'checking':
...
//# sourceMappingURL=exchangePlaidTokenMock.js.map