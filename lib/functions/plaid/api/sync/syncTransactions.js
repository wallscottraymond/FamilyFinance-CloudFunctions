...
const batch_create_transactions_1 = require("../../../transactions/utils/batch_create_transactions");
    ...
    while (hasMore) {
        try {
            ...
            if (data.added.length > 0) {
                ...
                const transactions = await (0, format_transactions_1.format_transactions)(data.added, itemId, userId, undefined, // groupId = null for now
                currency);
                console.log(`✅ Step 1/6: Formatted ${transactions.length} transactions`);
                // Step 2: Match transaction splits to categories (in-memory)
                const with_categories = await (0, match_categories_to_transactions_1.match_categories_to_transactions)(transactions, userId);
                ...
                const assignment_results = await (0, assign_transaction_splits_1.assign_transaction_splits_batch)(with_periods, userId);
                ...
                const { transactions: final, outflow_updates } = await (0, match_transaction_splits_to_outflows_1.match_transaction_splits_to_outflows)(with_budgets, userId);
                ...
                const count = await (0, batch_create_transactions_1.batch_create_transactions)(final, outflow_updates);
...
async function processModifiedTransactions(modifiedTransactions, itemId, userId) {
    ...
    try {
        ...
        if (materialChanges.length > 0) {
            ...
            const formatted_transactions = await (0, format_transactions_1.format_transactions)(materialChanges, itemId, userId, undefined, currency);
            console.log(`✅ Step 1/6: Formatted ${formatted_transactions.length} modified transactions`);
            // Step 2: Match categories
            const with_categories = await (0, match_categories_to_transactions_1.match_categories_to_transactions)(formatted_transactions, userId);
            console.log(`✅ Step 2/6: Matched categories for ${with_categories.length} transaction splits`);
            ...
            if (outflow_updates.length > 0) {
                ...
                for (const update of outflow_updates) {
                    const periodRef = index_1.db.collection('outflow_periods').doc(update.period_id);
