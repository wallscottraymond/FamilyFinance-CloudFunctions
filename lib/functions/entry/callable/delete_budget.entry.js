...
{ maxInstances: 50, memory: "512MiB" }, async (request) => {
    var _a;
    if (!request.auth) {
    ...
    try {
        ...
        const result = await (0, budgets_1.delete_budget_orchestrator)(ctx, user_id, data.idempotency_key, data.budget_id, data.rollover_transfer_mode);
