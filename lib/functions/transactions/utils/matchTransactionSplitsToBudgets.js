...
async function matchTransactionSplitsToBudgets(transactions, userId) {
    ...
    try {
        ...
        transactions.forEach(transaction => {
            ...
            if (matchedBudget) {
                transaction.splits = transaction.splits.map(split => (Object.assign(Object.assign({}, split), { budgetId: matchedBudget.id, budgetName: matchedBudget.name, updatedAt: firestore_1.Timestamp.now() })));
                matchedCount++;
            }
            else {
