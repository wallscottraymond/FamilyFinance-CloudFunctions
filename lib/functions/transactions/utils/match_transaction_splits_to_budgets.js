...
exports.match_transaction_splits_to_budgets = match_transaction_splits_to_budgets;
exports.matchTransactionSplitsToBudgets = match_transaction_splits_to_budgets;
 ...
 */
function does_category_match(split, budget) {
 ...
 */
async function match_transaction_splits_to_budgets(transactions, user_id) {
    ...
    try {
        ...
        budget_docs.forEach((doc) => {
            ...
            budgets.push({
                ...
                start_date: budget_data.startDate ? budget_data.startDate.toMillis() : null,
                end_date: budget_data.endDate ? budget_data.endDate.toMillis() : null,
        ...
        transactions.forEach(transaction => {
            // B1: Income must NEVER auto-assign to a budget. Recurring income was
            // leaking into "Everything Else" and inflating spend. Income counts toward
            // a budget ONLY when the user explicitly assigns it (manualBudgetAssignment
            // — see B2). Here we skip auto-matching and reset any prior auto-assignment
            // back to 'unassigned', so a backfill also cleans up already-leaked income.
            if (transaction.type === types_1.TransactionType.INCOME) {
                transaction.splits = transaction.splits.map(split => split.manualBudgetAssignment
                    ? split
                    : Object.assign(Object.assign({}, split), { budgetId: 'unassigned', updatedAt: firestore_1.Timestamp.now() }));
                return;
            }
            ...
            transaction.splits = transaction.splits.map(split => {
                ...
                if (matched_budget) {
                    ...
                    return Object.assign(Object.assign({}, split), { budgetId: matched_budget.id, budgetName: matched_budget.name, updatedAt: firestore_1.Timestamp.now() });
                }
                else {
...
//# sourceMappingURL=match_transaction_splits_to_budgets.js.map