...
exports.validateAndFixBudgetIds = validateAndFixBudgetIds;
exports.isValidBudgetId = isValidBudgetId;
 ...
 */
async function validateAndFixBudgetIds(userId, splits) {
    ...
    try {
        ...
        const budgetNames = new Map(); // Map budgetId -> budgetName
        ...
        const fixedSplits = splits.map(split => {
            ...
            if (!validBudgetIds.has(split.budgetId)) {
                console.warn(`[validateBudgetIds] Invalid budgetId: ${split.budgetId} (split: ${split.splitId}) - ` +
                    `auto-fixing to ${everythingElseBudgetId || 'unassigned'}`);
                fixedCount++;
                return Object.assign(Object.assign({}, split), { budgetId: everythingElseBudgetId || 'unassigned', budgetName: everythingElseBudgetId ? everythingElseBudgetName : undefined });
            }
            // Valid budgetId - set budgetName if not already set
            if (!split.budgetName && budgetNames.has(split.budgetId)) {
                return Object.assign(Object.assign({}, split), { budgetName: budgetNames.get(split.budgetId) });
            }
            return split;
        });
        if (fixedCount > 0) {
 ...
 */
async function isValidBudgetId(userId, budgetId) {
    ...
    try {
        ...
        return (budgetData === null || budgetData === void 0 ? void 0 : budgetData.userId) === userId && (budgetData === null || budgetData === void 0 ? void 0 : budgetData.isActive) === true;
    }
    catch (error) {
...
//# sourceMappingURL=validateBudgetIds.js.map