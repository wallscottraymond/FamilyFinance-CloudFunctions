...
exports.validate_and_fix_budget_ids = validate_and_fix_budget_ids;
exports.validateAndFixBudgetIds = validate_and_fix_budget_ids;
exports.is_valid_budget_id = is_valid_budget_id;
exports.isValidBudgetId = is_valid_budget_id;
 ...
 */
async function validate_and_fix_budget_ids(user_id, splits) {
    ...
    try {
        ...
        const valid_budget_ids = new Set();
        const budget_names = new Map(); // Map budgetId -> budgetName
        ...
        for (let i = 0; i < budget_ids.length; i += 30) {
            ...
            const budgets_snapshot = await db.collection('budgets')
                ...
                .where('isActive', '==', true)
                .where(firestore_1.FieldPath.documentId(), 'in', batch_ids)
                .get();
        ...
        const fixed_splits = splits.map(split => {
            ...
            if (!valid_budget_ids.has(split.budgetId)) {
                console.warn(`[validate_budget_ids] Invalid budgetId: ${split.budgetId} (split: ${split.splitId}) - ` +
                    `auto-fixing to ${everything_else_budget_id || 'unassigned'}`);
                fixed_count++;
                return Object.assign(Object.assign({}, split), { budgetId: everything_else_budget_id || 'unassigned', budgetName: everything_else_budget_id ? everything_else_budget_name : undefined });
            }
            // Valid budgetId - set budgetName if not already set
            if (!split.budgetName && budget_names.has(split.budgetId)) {
                return Object.assign(Object.assign({}, split), { budgetName: budget_names.get(split.budgetId) });
            }
            return split;
        });
        if (fixed_count > 0) {
 ...
 */
async function is_valid_budget_id(user_id, budget_id) {
    ...
    try {
        ...
        return (budget_data === null || budget_data === void 0 ? void 0 : budget_data.userId) === user_id && (budget_data === null || budget_data === void 0 ? void 0 : budget_data.isActive) === true;
    }
    catch (error) {
...
//# sourceMappingURL=validate_budget_ids.js.map