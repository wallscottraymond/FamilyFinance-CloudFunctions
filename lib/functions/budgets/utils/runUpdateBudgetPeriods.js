...
exports.runUpdateBudgetPeriods = runUpdateBudgetPeriods;
 ...
 */
function isCurrentOrFuturePeriod(period) {
 ...
 */
async function getSourcePeriod(db, sourcePeriodId, cache) {
 ...
 */
async function runUpdateBudgetPeriods(db, budgetId, budgetBefore, budgetAfter) {
    const result = {
        success: false,
        periodsQueried: 0,
        periodsUpdated: 0,
    ...
    try {
        ...
        let updatedCount = 0;
        const sourcePeriodCache = new Map();
        ...
        const budgetPeriodType = budgetAfter.period === 'weekly' ? types_1.PeriodType.WEEKLY :
            budgetAfter.period === 'bi_monthly' ? types_1.PeriodType.BI_MONTHLY :
                types_1.PeriodType.MONTHLY;
        for (let i = 0; i < currentAndFuturePeriods.length; i += batchSize) {
            ...
            for (const periodDoc of batchPeriods) {
                ...
                if (changedFields.includes('amount')) {
                    ...
                    if (sourcePeriod) {
                        const newAllocatedAmount = (0, calculatePeriodAllocatedAmount_1.calculatePeriodAllocatedAmount)(budgetAfter.amount, budgetPeriodType, sourcePeriod);
                ...
                if (Object.keys(updates).length > 0) {
                    ...
                    updates.lastCalculated = firestore_1.Timestamp.now();
...
//# sourceMappingURL=runUpdateBudgetPeriods.js.map