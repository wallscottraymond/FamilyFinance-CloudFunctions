...
exports.generateBudgetPeriodsWithPrimeSystem = void 0;
...
var periodGenerationOrchestrator_1 = require("./periodGenerationOrchestrator");
Object.defineProperty(exports, "generateBudgetPeriodsWithPrimeSystem", { enumerable: true, get: function () { return periodGenerationOrchestrator_1.generateBudgetPeriodsWithPrimeSystem; } });
/**
...
async function createBudgetPeriodsFromSource(db, budgetId, budget, startDate, endDate) {
    ...
    sourcePeriodsSnapshot.forEach((doc) => {
        ...
        const budgetPeriodType = budget.period === 'weekly' ? types_1.PeriodType.WEEKLY :
            budget.period === 'bi_monthly' ? types_1.PeriodType.BI_MONTHLY :
...
async function updateBudgetPeriodRange(db, budgetId, firstPeriodId, lastPeriodId, endDate, isRecurring) {
    const updateData = {
        ...
        lastExtended: firestore_1.Timestamp.now(),
    };
    // Add metadata for recurring budgets to enable future extension
    if (isRecurring) {
        updateData.periodsGeneratedUntil = firestore_1.Timestamp.fromDate(endDate);
...
async function generateBudgetPeriodsForNewBudget(db, budgetId, budget) {
    ...
    const { generateBudgetPeriodsWithPrimeSystem } = await Promise.resolve().then(() => __importStar(require('./periodGenerationOrchestrator')));
