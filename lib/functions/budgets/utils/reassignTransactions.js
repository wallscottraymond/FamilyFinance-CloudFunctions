        ...
        for (const batch of batches) {
            ...
            for (const txnDoc of batch) {
                try {
                    ...
                    const originalSplits = JSON.stringify(txnData.splits.map((s) => s.budgetId));
                    ...
                    const matchedTransactions = await (0, matchTransactionSplitsToBudgets_1.matchTransactionSplitsToBudgets)([transactionToMatch], userId);
                    ...
                    const newSplits = JSON.stringify(updatedSplits.map((s) => s.budgetId));
                    const splitsChanged = originalSplits !== newSplits;
                    if (splitsChanged) {
                        // Count how many splits changed
                        for (let i = 0; i < updatedSplits.length; i++) {
                            if (((_b = txnData.splits[i]) === null || _b === void 0 ? void 0 : _b.budgetId) !== ((_c = updatedSplits[i]) === null || _c === void 0 ? void 0 : _c.budgetId)) {
