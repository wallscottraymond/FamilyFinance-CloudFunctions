...
async function getActiveCategories() {
    ...
    snapshot.forEach((doc) => {
        ...
        categories.push({
            ...
            name: data.name,
            type: data.type,
            firstCategoryId: (_a = data.firstCategoryId) !== null && _a !== void 0 ? _a : null,
            firstCategory: (_b = data.first_category) !== null && _b !== void 0 ? _b : null,
            overallCategoryId: (_c = data.overallCategoryId) !== null && _c !== void 0 ? _c : null,
        });
    });
    // Update cache
    categoriesCache = {
...
async function getCategoryOwnership(userId) {
    ...
    const ownershipByFirst = {};
    const firstByDetailed = new Map(); // detailed doc id → firstCategoryId
    const firstsByOverall = new Map(); // overallCategoryId → firstCategoryIds
    for (const cat of allCategories) {
        if (!cat.firstCategoryId)
            continue;
        ownershipByFirst[cat.firstCategoryId] = null;
        if (cat.firstCategory)
            firstCategoryNames[cat.firstCategoryId] = cat.firstCategory;
        firstByDetailed.set(cat.id, cat.firstCategoryId);
        if (cat.overallCategoryId) {
            let set = firstsByOverall.get(cat.overallCategoryId);
            if (!set) {
                set = new Set();
                firstsByOverall.set(cat.overallCategoryId, set);
            }
            set.add(cat.firstCategoryId);
        }
    }
    const firstsCoveredBy = (categoryId) => {
        if (ownershipByFirst.hasOwnProperty(categoryId))
            return [categoryId]; // a firstCategoryId
        if (firstsByOverall.has(categoryId))
            return [...firstsByOverall.get(categoryId)]; // an overall slug
