...
exports.releaseCategories = releaseCategories;
exports.transferCategories = transferCategories;
exports.bulkTransferCategories = bulkTransferCategories;
 ...
 */
async function claimCategories(userId, categoryIds, toBudgetId) {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            for (const [fromBudgetId, categoryIdsToRemove] of transfersBySource) {
                if (fromBudgetId) {
                    ...
                    transaction.update(fromBudgetRef, {
                        categoryIds: firestore_1.FieldValue.arrayRemove(...categoryIdsToRemove),
            ...
            transaction.update(toBudgetRef, {
                categoryIds: firestore_1.FieldValue.arrayUnion(...allCategoriesToAdd),
                updatedAt: firestore_1.Timestamp.now(),
            });
 ...
 * @returns TransferResult with details of the operation
 */
async function releaseCategories(userId, categoryIds, fromBudgetId) {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            transaction.update(fromBudgetRef, {
                categoryIds: firestore_1.FieldValue.arrayRemove(...categoryIds),
                updatedAt: firestore_1.Timestamp.now(),
            });
            // Add to Everything Else budget
            transaction.update(toBudgetRef, {
                categoryIds: firestore_1.FieldValue.arrayUnion(...categoryIds),
                updatedAt: firestore_1.Timestamp.now(),
            });
 ...
 * @returns TransferResult with details of the operation
 */
async function transferCategories(categoryIds, fromBudgetId, toBudgetId) {
 ...
 */
async function bulkTransferCategories(transfers) {
    ...
    try {
        ...
        const removals = new Map(); // budgetId → categoryIds
        const additions = new Map(); // budgetId → categoryIds
...
//# sourceMappingURL=categoryTransfer.js.map