...
exports.calculatePeriodAllocatedAmount = calculatePeriodAllocatedAmount;
exports.getDailyRate = getDailyRate;
 ...
 */
function calculateMonthlyToTarget(monthlyAmount, targetPeriod) {
    ...
    console.log(`[calculateMonthlyToTarget] Monthly: $${monthlyAmount}, ` +
 ...
 * ```
 */
function calculatePeriodAllocatedAmount(budgetAmount, budgetPeriodType, targetPeriod) {
    var _a, _b;
    ...
    switch (budgetPeriodType) {
        case types_1.PeriodType.MONTHLY:
            ...
            break;
        case types_1.PeriodType.BI_MONTHLY:
            ...
            const biMonthlyDays = ((_a = targetPeriod.metadata) === null || _a === void 0 ? void 0 : _a.biMonthlyHalf) === 1 ? 15 :
                (((_b = targetPeriod.metadata) === null || _b === void 0 ? void 0 : _b.month) ? getDaysInMonth(targetPeriod.year, targetPeriod.metadata.month - 1) - 15 : 15);
            const biMonthlyDailyRate = budgetAmount / biMonthlyDays;
            allocation = biMonthlyDailyRate * targetDays;
            console.log(`[calculatePeriodAllocatedAmount] Bi-monthly: $${budgetAmount} (${biMonthlyDays} days), ` +
                `Target: ${targetPeriod.id} (${targetDays} days), Daily rate: $${biMonthlyDailyRate.toFixed(2)}, ` +
                `Allocation: $${allocation.toFixed(2)}`);
            break;
        case types_1.PeriodType.WEEKLY:
            ...
            console.log(`[calculatePeriodAllocatedAmount] Weekly: $${budgetAmount}, ` +
                `Target: ${targetPeriod.id} (${targetDays} days), Daily rate: $${weeklyDailyRate.toFixed(2)}, ` +
                `Allocation: $${allocation.toFixed(2)}`);
            break;
        default:
            console.warn(`[calculatePeriodAllocatedAmount] Unknown budget period type: ${budgetPeriodType}, using target days`);
 ...
 */
function getDailyRate(budgetAmount, budgetPeriodType, periodDays) {
    switch (budgetPeriodType) {
        case types_1.PeriodType.MONTHLY:
...
//# sourceMappingURL=calculatePeriodAllocatedAmount.js.map