...
exports.createTestBudgetSuite = void 0;
 ...
 */
exports.createTestBudgetSuite = (0, https_1.onCall)(async (request) => {
    ...
    const { groupId, createTransactions = true, transactionAmount = 75, } = request.data || {};
    ...
    try {
        ...
        const now = new Date();
        const startDate = firestore_1.Timestamp.fromDate(new Date(now.getFullYear(), now.getMonth(), 1)); // First of current month
        const oneYearLater = new Date(startDate.toDate());
        oneYearLater.setFullYear(oneYearLater.getFullYear() + 1);
        const endDate = firestore_1.Timestamp.fromDate(oneYearLater);
        ...
        const budgetConfigs = [
            ...
            },
            {
                ...
                amount: 200,
                period: types_1.BudgetPeriod.MONTHLY, // Note: All budgets generate all period types
        ...
        for (const config of budgetConfigs) {
            ...
            console.log(`[createTestBudgetSuite] Created ${config.period} budget: ${budgetId}`);
            budgetResults.push({
                period: config.period,
                budgetId,
                budget: {
            ...
            periodResults.push({
                ...
                periodBreakdown: {
                    MONTHLY: periods.filter((p) => p.periodType === types_1.PeriodType.MONTHLY)
                        .length,
                    BI_MONTHLY: periods.filter((p) => p.periodType === types_1.PeriodType.BI_MONTHLY)
                        .length,
        ...
        if (createTransactions) {
            for (const result of budgetResults) {
                ...
                const transactionDate = firestore_1.Timestamp.now();
                ...
                console.log(`[createTestBudgetSuite] Created transaction for ${result.period}: ${transactionId}`);
            ...
            console.log("[createTestBudgetSuite] Waiting 3 seconds for spending updates...");
            await new Promise((resolve) => setTimeout(resolve, 3000));
        }
        // ===================================================================
        ...
        const summaries = summarySnapshot.docs.map((doc) => {
            ...
            const matchingBudgets = budgetResults.map((result) => {
                const entry = budgetEntries.find((b) => b.budgetId === result.budgetId);
                return {
                    period: result.period,
                    budgetId: result.budgetId,
                    found: !!entry,
        ...
        return {
            ...
            summary: {
                ...
                totalPeriodsGenerated: periodResults.reduce((sum, p) => sum + p.periodsGenerated, 0),
            },
            budgets: budgetResults.map((b, index) => (Object.assign(Object.assign({}, b), { periods: periodResults[index], transaction: createTransactions ? transactionResults[index] : null }))),
            userSummaries: {
                count: summaries.length,
                summaries: summaries.filter((s) => s.matchingBudgets.some((b) => b.found)),
            },
            verification: {
        ...
        console.error("[createTestBudgetSuite] Error creating test budget suite:", error);
        ...
        throw new https_1.HttpsError("internal", `Failed to create test budget suite: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
});
//# sourceMappingURL=createTestBudgetSuite.js.map