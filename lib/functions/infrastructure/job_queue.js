...
exports.reclaim_stuck_jobs = reclaim_stuck_jobs;
...
exports.purge_finished_jobs = purge_finished_jobs;
...
async function create_job(job_type, payload, options) {
    ...
    if (options === null || options === void 0 ? void 0 : options.dedup_key) {
 ...
 */
async function reclaim_stuck_jobs(stuck_after_ms = 15 * 60 * 1000, scan_limit = 500) {
    const db = (0, firestore_1.getFirestore)();
    ...
    const snapshot = await db
        ...
        .where("updated_at", "<=", firestore_1.Timestamp.fromMillis(cutoff_ms))
        .limit(scan_limit)
        .get();
    let reclaimed = 0;
...
async function mark_job_completed(job_id) {
    ...
    await db.collection(COLLECTIONS.JOBS).doc(job_id).update({
        status: "completed",
        updated_at: now,
        expire_at: firestore_1.Timestamp.fromMillis(now.toMillis() + COMPLETED_JOB_RETENTION_MS),
    });
}
/**
 ...
 */
async function purge_finished_jobs(options) {
    var _a, _b, _c;
    const db = (0, firestore_1.getFirestore)();
    const short_cutoff = firestore_1.Timestamp.fromMillis(Date.now() - ((_a = options === null || options === void 0 ? void 0 : options.completed_older_than_hours) !== null && _a !== void 0 ? _a : 24) * 60 * 60 * 1000);
    const failed_cutoff = firestore_1.Timestamp.fromMillis(Date.now() - ((_b = options === null || options === void 0 ? void 0 : options.failed_older_than_hours) !== null && _b !== void 0 ? _b : 24 * 7) * 60 * 60 * 1000);
    const max_deletes = (_c = options === null || options === void 0 ? void 0 : options.max_deletes) !== null && _c !== void 0 ? _c : 50000;
    const counts = {
        deleted: 0,
        completed: 0,
        cancelled: 0,
    ...
    const drain = async (status, cutoff) => {
    ...
    if (counts.deleted < max_deletes)
        await drain("cancelled", short_cutoff);
    if (counts.deleted < max_deletes)
        await drain("failed", failed_cutoff);
    if (counts.deleted >= max_deletes)
        counts.reached_cap = true;
    return counts;
}
...
function build_dedup_key(job_type, deduplication_key) {
...
async function create_job_if_not_exists(job_type, payload, options) {
    ...
    return create_job(job_type, payload, Object.assign(Object.assign({}, options), { dedup_key: build_dedup_key(job_type, payload.deduplication_key) }));
}
/**
