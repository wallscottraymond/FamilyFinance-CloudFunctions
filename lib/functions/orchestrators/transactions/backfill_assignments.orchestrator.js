...
async function backfill_assignments_orchestrator(ctx, input) {
    ...
    try {
        ...
        await enqueue_chunked(transaction_ids.map((transaction_id) => () => (0, job_queue_1.create_job)("assign_transaction", { user_id: input.user_id, transaction_id }, { trace_id: ctx.trace_id })));
