 ...
 */
function recurring_links_from_doc(doc) {
    var _a, _b, _c;
    const splits = (_a = doc.splits) !== null && _a !== void 0 ? _a : [];
    const out = (_b = doc.splitOutflowIds) !== null && _b !== void 0 ? _b : splits.map((s) => s.outflowId).filter((id) => !!id);
    const inf = (_c = doc.splitInflowIds) !== null && _c !== void 0 ? _c : splits.map((s) => s.inflowId).filter((id) => !!id);
    return {
 ...
 */
async function enqueue_recurring_reconciles(ctx, user_id, before, after, event_id) {
    const b = recurring_links_from_doc(before !== null && before !== void 0 ? before : {});
    const a = recurring_links_from_doc(after !== null && after !== void 0 ? after : {});
    ...
    for (const recurring_id of outflow_ids) {
        await (0, job_queue_1.create_job_if_not_exists)("reconcile_recurring_period", {
...
async function process_transaction_written_orchestrator(ctx, input) {
    ...
    if (assignment_relevant) {
        await (0, job_queue_1.create_job_if_not_exists)("assign_user_transactions", {
