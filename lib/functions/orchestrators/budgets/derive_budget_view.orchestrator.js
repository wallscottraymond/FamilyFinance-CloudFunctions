...
async function derive_budget_view_orchestrator(ctx, user_id, input) {
    ...
    try {
        ...
        if (budget.is_system_everything_else === true) {
            const period_result = await (0, derive_period_orchestrator_1.derive_period_orchestrator)(ctx, user_id, {
                view_cadence: input.view_cadence,
                window_start_ms: input.window_start_ms,
                window_end_ms: input.window_end_ms,
            });
            const ee = period_result.budgets.find((b) => b.is_everything_else && b.budget_id === input.budget_id);
            (0, observability_1.log_operation_success)(span, user_id);
            return {
                budget_id: input.budget_id,
                budget_name: budget.name,
                view_cadence: input.view_cadence,
                periods: (_a = ee === null || ee === void 0 ? void 0 : ee.periods) !== null && _a !== void 0 ? _a : [],
            };
        }
        // 2. Gather derivation inputs (buckets + monthly periods + splits), bounded
        //    to the window.
        const deps = await (0, budget_view_resolver_1.resolve_budget_view_deps)(ctx, user_id, input.budget_id, input.view_cadence, input.window_start_ms, input.window_end_ms, (_b = input.match_mode) !== null && _b !== void 0 ? _b : "stored", false // EE returned early above (its limit = derived leftover via derive_period)
        );
