...
async function rehome_claimed_transactions(ctx, payload) {
    ...
    if (txn_ids.length > 0) {
        await (0, job_queue_1.create_job)("assign_transactions_batch", { user_id: payload.user_id, transaction_ids: txn_ids }, { trace_id: ctx.trace_id });
...
async function generate_periods(ctx, payload) {
    ...
    const entities = payload.prime_only
        ...
        : computed.entities;
    await budget_period_repo_1.budget_period_repo.save_batch(ctx, entities, payload.budget_name);
