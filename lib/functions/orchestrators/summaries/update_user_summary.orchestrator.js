...
const summaries_1 = require("../../domain/summaries");
...
async function update_user_summary_orchestrator(ctx) {
    ...
    try {
        ...
        await user_summary_repo_1.user_summary_repo.save_with_transaction(ctx, summary_id, ctx.input.user_id, ctx.input.source_period_id, ctx.input.period_type, (deps) => {
            ...
            const compute_result = (0, summaries_1.compute_user_period_summary)({
            ...
            const validation_result = (0, summaries_1.validate_user_period_summary)(summary);
 ...
 */
async function enqueue_summary_update_jobs(_ctx, _user_id, _periods_by_type) {
 ...
 * @returns Count of jobs enqueued
 */
async function enqueue_user_summary_updates_from_outflow_periods(_ctx, _user_id, _outflow_period_ids) {
 ...
 */
async function enqueue_user_summary_updates_from_inflow_periods(_ctx, _user_id, _inflow_period_ids) {
 ...
 * @returns Count of jobs enqueued
 */
async function enqueue_user_summary_updates_from_budget_periods(_ctx, _user_id, _budget_period_ids) {
    // RETIRED (see enqueue_summary_update_jobs): the summary build is off, so resolving these period
