...
exports.GENERATE_OUTFLOW_PERIODS_BUDGET = void 0;
exports.generate_outflow_periods_orchestrator = generate_outflow_periods_orchestrator;
 ...
 */
exports.GENERATE_OUTFLOW_PERIODS_BUDGET = {
    max_reads: 50,
 ...
 */
async function generate_outflow_periods_orchestrator(ctx) {
    var _a, _b;
    const start_time = Date.now();
    const perf = (0, types_1.create_performance_metrics)();
    const errors = [];
    // Create span for this orchestrator
    const span = (0, observability_1.create_span)(ctx, "orchestrator", "generate_outflow_periods");
    (0, observability_1.log_operation_start)(span, ctx.input.user_id);
    try {
        // 1. RESOLVE DEPENDENCIES
        const resolve_span = (0, observability_1.create_span)((0, observability_1.create_child_span)(ctx), "resolver", "resolve_outflow_period_dependencies_from_doc");
        let deps;
        try {
            deps = await (0, outflows_1.resolve_outflow_period_dependencies_from_doc)(resolve_span, ctx.input.outflow_id, ctx.input.outflow_data);
        ...
        if (deps.source_periods.length === 0) {
            console.warn(`[${ctx.trace_id}] generate_outflow_periods: No source periods found, skipping`);
        ...
        console.log(`[${ctx.trace_id}] generate_outflow_periods: ${deps.source_periods.length} source periods`);
        // 2. COMPUTE OUTFLOW PERIODS (Domain Service - PURE)
        const now = firestore_1.Timestamp.now();
        const compute_result = (0, outflows_2.compute_outflow_periods)(deps.outflow, deps.source_periods, now);
        ...
        console.log(`[${ctx.trace_id}] generate_outflow_periods: computed ${periods.length} periods`);
        // 3. VALIDATE PERIODS (Domain Service - PURE)
        const validation_result = (0, outflows_2.validate_outflow_periods)(periods);
        ...
        const persist_span = (0, observability_1.create_span)((0, observability_1.create_child_span)(ctx), "repository", "outflow_period_repo.save_batch");
        const write_result = await outflow_period_repo_1.outflow_period_repo.save_batch(persist_span, periods, ctx.input.user_id);
        ...
        if (write_result.count > 0) {
            ...
            console.log(`[${ctx.trace_id}] generate_outflow_periods: enqueue ${period_ids.length} summaries`);
            try {
                const summaries_updated = await (0, summaries_1.enqueue_user_summary_updates_from_outflow_periods)(ctx, ctx.input.user_id, period_ids);
                console.log(`[${ctx.trace_id}] generate_outflow_periods: updated ${summaries_updated} summaries`);
                perf.writes += summaries_updated;
            }
            catch (summary_error) {
                // Non-fatal: log but don't fail the orchestrator
                console.error(`[${ctx.trace_id}] generate_outflow_periods: summary update failed (non-fatal):`, summary_error);
            }
        }
        // 6. CHECK PERFORMANCE BUDGET
        perf.time_ms = Date.now() - start_time;
        if ((0, types_1.is_budget_exceeded)(perf, exports.GENERATE_OUTFLOW_PERIODS_BUDGET)) {
            console.warn(`[${ctx.trace_id}] generate_outflow_periods: Performance budget exceeded`, { perf, budget: exports.GENERATE_OUTFLOW_PERIODS_BUDGET });
        }
        // 7. LOG SUCCESS
        (0, observability_1.log_operation_success)(span, ctx.input.user_id);
        console.log(`[${ctx.trace_id}] generate_outflow_periods: created ${write_result.count} periods ` +
...
//# sourceMappingURL=generate_outflow_periods.orchestrator.js.map