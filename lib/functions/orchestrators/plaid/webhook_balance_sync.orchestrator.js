...
async function webhook_balance_sync_orchestrator(ctx) {
    ...
    if (!deps.item) {
        ...
        await (0, webhook_balance_sync_resolver_1.record_webhook_processed)(ctx, ctx.input, false, "Item not found");
        return {
            success: false,
            accounts_created: 0,
            accounts_updated: 0,
    ...
    let plaid_result;
    try {
        plaid_result = await (0, plaid_2.fetch_plaid_balances)(deps.item.access_token);
    }
    catch (error) {
        ...
        // Record webhook as failed
        await (0, webhook_balance_sync_resolver_1.record_webhook_processed)(ctx, ctx.input, false, `Plaid API error: ${error_msg}`);
        return {
            success: false,
            accounts_created: 0,
            accounts_updated: 0,
    ...
    if (plaid_result.accounts.length === 0) {
        ...
        await (0, webhook_balance_sync_resolver_1.record_webhook_processed)(ctx, ctx.input, true, "No accounts returned from Plaid");
    ...
    try {
        ...
        // Record webhook as failed
        await (0, webhook_balance_sync_resolver_1.record_webhook_processed)(ctx, ctx.input, false, `Database error: ${error_msg}`);
        return {
            success: false,
            accounts_created: 0,
            accounts_updated: 0,
    ...
    await (0, webhook_balance_sync_resolver_1.record_webhook_processed)(ctx, ctx.input, true, `Synced: ${accounts_created} created, ${accounts_updated} updated`);
