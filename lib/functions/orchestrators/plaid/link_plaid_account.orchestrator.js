...
async function link_plaid_account_orchestrator(ctx) {
    ...
    try {
        ...
        try {
            const institution = await (0, plaid_3.get_institution_by_id)(ctx.input.institution_id);
            institution_logo = institution.logo;
        }
        catch (logo_error) {
            console.warn(`[${ctx.trace_id}] Could not fetch institution logo for ${ctx.input.institution_id}:`, logo_error instanceof Error ? logo_error.message : logo_error);
        }
        const plaid_item = {
