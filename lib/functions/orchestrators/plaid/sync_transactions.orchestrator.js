...
const rule_evaluation_service_1 = require("../../domain/rules/rule_evaluation.service");
const rule_application_service_1 = require("../../domain/rules/rule_application.service");
...
async function sync_transactions_orchestrator(ctx) {
    ...
    const active_rules = await rules_repo_1.rules_repo.get_active_rules((0, observability_1.create_child_span)(ctx), ctx.user_id);
    const on_create = active_rules.length > 0
        ? (t) => (0, rule_application_service_1.apply_rule_intents)(t, (0, rule_evaluation_service_1.evaluate_rules)(t, active_rules))
        : undefined;
    ...
    while (has_more && (0, plaid_5.should_continue_sync)(errors)) {
        ...
        if (active_added.length > 0) {
            try {
                const page_result = await process_added_transactions(ctx, active_added, deps, errors, on_create);
    ...
    if (total_added + total_modified + total_removed > 0) {
        await (0, derive_version_repo_1.bump_derive_version)(ctx.user_id).catch(() => { });
    }
    // 3. UPDATE CURSOR
    try {
 ...
 */
async function process_added_transactions(ctx, plaid_transactions, deps, errors, on_create) {
    ...
    if (plaid_transactions.length > 0) {
        try {
            ...
            const upsert_result = await repositories_1.transaction_repo.upsert_from_plaid_sync((0, observability_1.create_child_span)(ctx), transactions_for_persistence, ctx.user_id, deps.plaid_item.plaid_item_id, on_create);
