...
async function sync_balances_orchestrator(ctx) {
    ...
    for (const item of deps.items) {
        ...
        const raw_liabilities = await (0, plaid_2.safe_fetch_liabilities)(item.access_token, ctx.trace_id);
        const liabilities = (0, plaid_2.plaid_liabilities_to_domain)(raw_liabilities);
        // 2b. UPSERT ACCOUNTS (creates if new, updates balances if exists)
        try {
            const upsert_result = await account_repo_1.account_repo.upsert_from_plaid((0, observability_1.create_child_span)(ctx), plaid_result.accounts, item.item_id, ctx.user_id, { id: item.institution_id, name: item.institution_name }, item.group_id, liabilities);
            ...
            for (const result of upsert_result.results) {
                ...
                const balance_changed = result.action === "updated" &&
    ...
    if (snapshots_to_append.length > 0) {
        try {
            await balance_snapshot_repo_1.balance_snapshot_repo.append(ctx, snapshots_to_append);
        }
        catch (error) {
            console.error(`[${ctx.trace_id}] Failed to append balance snapshots:`, error);
        }
    }
