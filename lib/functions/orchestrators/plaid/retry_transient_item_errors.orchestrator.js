    ...
    try {
        ...
        for (const item of items) {
            ...
            if (action === "recovered") {
                ...
                await plaid_1.plaid_item_repo.apply_field_update(ctx, item.item_doc_id, {
                ...
                const update = (0, item_status_webhook_service_1.compute_escalation_update)(now, item.error_code);
                await plaid_1.plaid_item_repo.apply_field_update(ctx, item.item_doc_id, {
                    status: update.status,
                ...
                await plaid_1.plaid_item_repo.apply_field_update(ctx, item.item_doc_id, patch);
