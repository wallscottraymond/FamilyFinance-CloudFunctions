...
async function plaid_initial_sync_orchestrator(ctx) {
    ...
    try {
        ...
        try {
            await (0, job_queue_1.create_job)("resync_transactions", { item_id: ctx.input.item_doc_id, user_id: ctx.user_id, attempt: 1 }, { delay_seconds: 120, trace_id: ctx.trace_id });
            console.log(`[${ctx.trace_id}] Scheduled delayed transaction re-sync for item ${ctx.input.item_doc_id}`);
        }
        catch (error) {
            console.warn(`[${ctx.trace_id}] Failed to schedule delayed transaction re-sync:`, error instanceof Error ? error.message : "unknown");
        }
        // Check if we should continue
...
async function execute_accounts_phase(ctx, deps, events, perf) {
    ...
    try {
        ...
        const raw_accounts = await (0, plaid_4.fetch_plaid_balances)(deps.plaid_item.access_token);
        const plaid_result = Object.assign(Object.assign({}, raw_accounts), { accounts: (0, plaid_4.plaid_accounts_to_data)(raw_accounts.accounts) });
        ...
        const raw_liabilities = await (0, plaid_4.safe_fetch_liabilities)(deps.plaid_item.access_token, ctx.trace_id);
        const liabilities = (0, plaid_4.plaid_liabilities_to_domain)(raw_liabilities);
        ...
        const upsert_result = await account_repo_1.account_repo.upsert_from_plaid({ trace_id: ctx.trace_id, span_id: ctx.span_id }, plaid_result.accounts, ctx.input.plaid_item_id, ctx.user_id, { id: ctx.input.institution_id, name: ctx.input.institution_name }, group_id, liabilities);
