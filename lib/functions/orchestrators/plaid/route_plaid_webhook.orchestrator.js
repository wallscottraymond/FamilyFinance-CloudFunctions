...
async function route_plaid_webhook_orchestrator(ctx, input) {
    ...
    if (plaid_item_id) {
        try {
            await plaid_item_repo_1.plaid_item_repo.mark_webhook_received({ trace_id, span_id }, plaid_item_id);
        }
        catch (err) {
            console.warn(`[${trace_id}] Failed to stamp lastWebhookReceived for ${plaid_item_id}:`, err);
        }
    }
    // ITEM webhooks
    ...
    if (webhook_type === types_1.PlaidWebhookType.TRANSACTIONS) {
        switch (webhook_code) {
            ...
            case types_1.PlaidWebhookCode.INITIAL_UPDATE: {
                ...
                if (await (0, purge_guard_1.is_user_purging)(deps.plaid_item.user_id)) {
