...
exports.user_summary_repo = {
    ...
    async save_with_transaction(ctx, summary_id, user_id, source_period_id, period_type, compute_fn) {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            const [outflow_snapshot, budget_snapshot, inflow_snapshot, outflow_defs_snapshot, inflow_defs_snapshot, budget_defs_snapshot,] = await Promise.all([
            ...
            const canonical_ee_id = (_d = (_b = (_a = ee_defs.find((b) => b.data.period === "monthly")) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : (_c = ee_defs[0]) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : null;
            const excluded_ee_ids = new Set(ee_defs.map((b) => b.id).filter((id) => id !== canonical_ee_id));
            const hidden_outflow_ids = new Set(outflow_defs_snapshot.docs
                .filter((d) => d.data().isHidden === true)
                .map((d) => d.id));
            const hidden_inflow_ids = new Set(inflow_defs_snapshot.docs
                .filter((d) => d.data().isHidden === true)
                .map((d) => d.id));
