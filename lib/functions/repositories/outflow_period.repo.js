...
exports.outflow_period_repo = {
     ...
     */
    async get_in_due_window_for_ids(_ctx, outflow_ids, start_ms, end_ms) {
     ...
     */
    async set_hidden_by_outflow_ids(ctx, outflow_ids, hidden) {
        if (outflow_ids.length === 0) {
            return 0;
        }
        const db = (0, firestore_1.getFirestore)();
        ...
        console.log(`[${ctx.trace_id}] outflow_period.set_hidden_by_outflow_ids: ` +
    ...
    async update_reconciliation(_ctx, results) {
        ...
        for (const chunk of (0, types_1.chunk_for_batch)(results)) {
            ...
            for (const r of chunk) {
                ...
                // Count-but-flag-pending: pending payments count toward the paid dollar +
                // occurrences (so the item doesn't look unpaid); `hasPending` flags that some
                // of it is still pending so the tile can mark it. Posted vs pending split is
                // kept in `reconciliation.matchedAmount` / `pendingAmount`.
                const counted_paid = r.matched_amount + r.pending_amount;
                /* eslint-disable @typescript-eslint/naming-convention */
                const update_data = {
                    reconciliation: {
                        status: r.status,
                    ...
                    hasPending: r.has_pending,
                    amountPaid: counted_paid,
                    // Dollar amounts the user_summary / tiles read (paid vs expected).
                    totalAmountDue: r.expected_amount,
                    totalAmountPaid: counted_paid,
                    totalAmountUnpaid: Math.max(0, r.expected_amount - counted_paid),
