...
exports.transaction_repo = {
     ...
     */
    async get_ids_by_account_id(ctx, account_id, user_id, limit = 1000) {
        const db = (0, firestore_1.getFirestore)();
        const snapshot = await db
            .collection(COLLECTION)
            .where("accountId", "==", account_id)
     ...
     */
    async update_plaid_item_cursor(ctx, item_doc_id, cursor) {
        const db = (0, firestore_1.getFirestore)();
        const now = firestore_1.Timestamp.now();
        ...
        console.log(`[${ctx.trace_id}] update_plaid_item_cursor: item=${item_doc_id}, cursor=${cursor ? "updated" : "cleared"}`);
    },
};
