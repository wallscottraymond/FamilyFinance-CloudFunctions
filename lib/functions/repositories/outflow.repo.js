...
function map_removal_intervals(docs) {
    return (docs !== null && docs !== void 0 ? docs : []).map((d) => ({ from_ms: d.fromMs, to_ms: d.toMs, mode: d.mode }));
}
/** Map domain intervals → stored interval docs. */
function to_removal_interval_docs(intervals) {
...
function map_to_entity(doc) {
    ...
    return {
        ...
        expected_amount_override: (_b = doc.expectedAmountOverride) !== null && _b !== void 0 ? _b : null,
        ...
        expense_type: (_c = doc.expenseType) !== null && _c !== void 0 ? _c : "other",
        is_essential: (_d = doc.isEssential) !== null && _d !== void 0 ? _d : false,
        type: doc.type,
        status: "active",
        source: doc.source,
        plaid_status: (_e = doc.plaidStatus) !== null && _e !== void 0 ? _e : "UNKNOWN",
        plaid_confidence_level: (_f = doc.plaidConfidenceLevel) !== null && _f !== void 0 ? _f : null,
        is_hidden: doc.isHidden,
        is_user_modified: doc.isUserModified,
        transaction_ids: doc.transactionIds,
        tags: doc.tags,
        rules: doc.rules,
        removed_by_user: (_g = doc.removedByUser) !== null && _g !== void 0 ? _g : false,
...
function map_persistence_to_doc(entity, now, existing) {
    ...
    return {
        ...
        expectedAmountOverride: (_c = existing === null || existing === void 0 ? void 0 : existing.expectedAmountOverride) !== null && _c !== void 0 ? _c : null,
        currency: entity.currency,
        description: entity.description,
        merchantName: entity.merchant_name,
        userCustomName: (_d = existing === null || existing === void 0 ? void 0 : existing.userCustomName) !== null && _d !== void 0 ? _d : entity.user_custom_name,
        ...
        internalPrimaryCategory: (_e = existing === null || existing === void 0 ? void 0 : existing.internalPrimaryCategory) !== null && _e !== void 0 ? _e : entity.internal_primary_category,
        internalDetailedCategory: (_f = existing === null || existing === void 0 ? void 0 : existing.internalDetailedCategory) !== null && _f !== void 0 ? _f : entity.internal_detailed_category,
        expenseType: entity.expense_type,
        isEssential: entity.is_essential,
        type: (_g = existing === null || existing === void 0 ? void 0 : existing.type) !== null && _g !== void 0 ? _g : "recurring",
        source: entity.source,
        plaidStatus: entity.plaid_status,
        plaidConfidenceLevel: entity.plaid_confidence_level,
        isHidden: (_h = existing === null || existing === void 0 ? void 0 : existing.isHidden) !== null && _h !== void 0 ? _h : entity.is_hidden,
        isUserModified: (_j = existing === null || existing === void 0 ? void 0 : existing.isUserModified) !== null && _j !== void 0 ? _j : entity.is_user_modified,
        transactionIds: entity.transaction_ids,
        tags: (_k = existing === null || existing === void 0 ? void 0 : existing.tags) !== null && _k !== void 0 ? _k : entity.tags,
        rules: (_l = existing === null || existing === void 0 ? void 0 : existing.rules) !== null && _l !== void 0 ? _l : entity.rules,
        // Preserve user remove/pause state across re-sync (Plaid never sets these).
        removedByUser: (_m = existing === null || existing === void 0 ? void 0 : existing.removedByUser) !== null && _m !== void 0 ? _m : false,
        removalIntervals: (_o = existing === null || existing === void 0 ? void 0 : existing.removalIntervals) !== null && _o !== void 0 ? _o : [],
        lastSyncedAt: now,
    };
    /* eslint-enable @typescript-eslint/naming-convention */
}
...
exports.outflow_repo = {
     ...
     */
    async set_removal_intervals(ctx, id, intervals, removed_by_user, user_id) {
        ...
        const now = firestore_1.Timestamp.now();
        /* eslint-disable @typescript-eslint/naming-convention */
        const after = Object.assign(Object.assign({}, before), { removedByUser: removed_by_user, removalIntervals: to_removal_interval_docs(intervals), updatedAt: now });
        /* eslint-enable @typescript-eslint/naming-convention */
        ...
        return (0, types_1.create_write_result)("outflow", id, "replace", before, after);
    },
    /**
     ...
     */
    async apply_period_suppression(ctx, id, intervals, user_id) {
        var _a;
        const db = (0, firestore_1.getFirestore)();
        ...
        for (const doc of snap.docs) {
            const p = doc.data();
            const end_ms = (_a = p.periodEndDate) === null || _a === void 0 ? void 0 : _a.toMillis();
            if (end_ms === undefined)
                continue;
            const next_active = !(0, recurring_suppression_service_1.is_suppressed_in_period)(intervals, end_ms);
            if ((p.isActive !== false) === next_active)
        ...
        if (batched > 0)
            await batch.commit();
        console.log(`[${ctx.trace_id}] outflow_repo.apply_period_suppression ${id}: flipped ${flipped}/${snap.size} periods (user ${user_id})`);
        return flipped;
    },
    /**
    ...
    async hard_delete(ctx, id, user_id) {
        ...
        return (0, types_1.create_write_result)("outflow", id, "replace", before, null);
    },
    /**
    ...
    async get_by_account_id(ctx, account_id, options) {
        ...
        const snapshot = await query.get();
        const outflows = snapshot.docs.map((doc) => map_to_entity(doc.data()));
        console.log(`[${ctx.trace_id}] get_by_account_id: account=${account_id}, found=${outflows.length}`);
        return outflows;
    },
    /**
    ...
    async save_batch(ctx, entities) {
        ...
        for (const chunk of chunks) {
            ...
            for (const entity of chunk) {
                ...
                results.push((0, types_1.create_write_result)("outflow", entity.id, existing ? "replace" : "replace", existing_doc, doc_data));
                // Audit entry (async)
                (0, audit_1.record_audit_entry_async)({
        ...
        console.log(`[${ctx.trace_id}] outflow_repo.save_batch: saved=${results.length}`);
        return {
            results,
    ...
    async mark_stale(ctx, stream_ids, user_id) {
        ...
        for (const chunk of chunks) {
            ...
            for (const id of chunk) {
                ...
                results.push((0, types_1.create_write_result)("outflow", id, "replace", before, after));
                (0, audit_1.record_audit_entry_async)({
                    user_id,
                    action: "update",
                    entity_type: "recurring_outflow",
     ...
     */
    async mark_hidden(ctx, stream_ids, hidden, user_id) {
        if (stream_ids.length === 0) {
            return [];
        }
        const db = (0, firestore_1.getFirestore)();
        const now = firestore_1.Timestamp.now();
        const results = [];
        for (const chunk of (0, types_1.chunk_for_batch)(stream_ids)) {
            const batch = db.batch();
            for (const id of chunk) {
                const before_doc = await doc_ref(id).get();
                if (!before_doc.exists)
                    continue;
                const before = before_doc.data();
                if (before.isHidden === hidden)
                    continue; // idempotent
                // eslint-disable-next-line @typescript-eslint/naming-convention
                const after = Object.assign(Object.assign({}, before), { isHidden: hidden, updatedAt: now });
                batch.set(doc_ref(id), after);
                results.push((0, types_1.create_write_result)("outflow", id, "replace", before, after));
                (0, audit_1.record_audit_entry_async)({
                    user_id,
...
//# sourceMappingURL=outflow.repo.js.map