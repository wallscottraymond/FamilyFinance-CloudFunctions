...
exports.budget_period_repo = {
     ...
     */
    async get_by_budget_id_in_date_window(_ctx, budget_id, date_ms) {
        ...
        const snapshot = await (0, firestore_1.getFirestore)()
            .collection(COLLECTION)
            .where("budgetId", "==", budget_id)
            .where("periodStart", ">=", firestore_1.Timestamp.fromMillis(lo_ms))
            .where("periodStart", "<=", firestore_1.Timestamp.fromMillis(date_ms))
            .get();
        return snapshot.docs.map((doc) => map_to_entity(doc.data()));
    },
    /**
     ...
     */
    async get_by_user_and_type(_ctx, user_id, period_type) {
        const snapshot = await (0, firestore_1.getFirestore)()
            .collection(COLLECTION)
    ...
    async get_by_ids(_ctx, period_ids) {
        ...
        const snapshot_groups = await Promise.all(chunks.map((chunk) => db.getAll(...chunk.map((id) => collection.doc(id)))));
        ...
        for (const chunk of chunks) {
            ...
            for (const u of chunk) {
                ...
                const update_data = {
                    ...
                    returnAmount: (_a = u.return_amount) !== null && _a !== void 0 ? _a : 0,
                    lastCalculated: now,
                    updatedAt: now,
                };
                /* eslint-enable @typescript-eslint/naming-convention */
     ...
     */
    async get_pending_rollover_by_type(_ctx, budget_id) {
        const snapshot = await (0, firestore_1.getFirestore)()
            .collection(COLLECTION)
            .where("budgetId", "==", budget_id)
            .where("isActive", "==", true)
        ...
        snapshot.forEach((doc) => {
            var _a, _b, _c, _d;
            const d = doc.data();
            const pending = (_a = d.pendingRolloverDeduction) !== null && _a !== void 0 ? _a : 0;
            const periods = (_b = d.pendingRolloverPeriods) !== null && _b !== void 0 ? _b : 0;
            if (pending > 0 && periods > 0) {
                const type = (_c = d.periodType) !== null && _c !== void 0 ? _c : "monthly";
                const cur = (_d = by_type.get(type)) !== null && _d !== void 0 ? _d : { amount: 0, periods: 0 };
                cur.amount += pending;
     ...
     */
    async transfer_rollover_to_budget(_ctx, target_budget_id, transfers, mode) {
        var _a, _b, _c, _d;
        ...
        for (const t of transfers) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const share = i === n - 1 ? round2(t.amount - round2(t.amount / n) * (n - 1)) : round2(t.amount / n);
                const p = periods[i];
                const d = p.data;
                const new_rolled = round2(((_a = d.rolledOverAmount) !== null && _a !== void 0 ? _a : 0) - share);
                const allocated = (_c = (_b = d.modifiedAmount) !== null && _b !== void 0 ? _b : d.allocatedAmount) !== null && _c !== void 0 ? _c : 0;
                const remaining = round2(allocated + new_rolled - ((_d = d.spent) !== null && _d !== void 0 ? _d : 0));
                /* eslint-disable @typescript-eslint/naming-convention */
