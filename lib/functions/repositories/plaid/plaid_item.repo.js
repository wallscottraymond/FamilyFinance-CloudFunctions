...
exports.plaid_item_repo = {
     ...
     */
    async get_active_raw_by_plaid_item_id(_ctx, plaid_item_id) {
    ...
    async get_in_transient_state(ctx, statuses) {
        ...
        for (const doc of snapshot.docs) {
            ...
            rows.push({
                ...
                retry_count: (_c = raw.retryCount) !== null && _c !== void 0 ? _c : 0,
            });
        }
        return rows;
    },
     ...
     */
    async apply_field_update(ctx, id, fields) {
        /* eslint-disable @typescript-eslint/naming-convention */
        await doc_ref(id).update(Object.assign(Object.assign({}, fields), { updatedAt: firestore_1.FieldValue.serverTimestamp() }));
        /* eslint-enable @typescript-eslint/naming-convention */
     ...
     */
    async mark_webhook_received(ctx, plaid_item_id) {
        const snapshot = await get_db()
            .collection(COLLECTION)
            .where("plaidItemId", "==", plaid_item_id)
            .limit(1)
            .get();
        if (snapshot.empty) {
            console.warn(`[${ctx.trace_id}] mark_webhook_received: no item for plaidItemId=${plaid_item_id}`);
            return;
        }
        /* eslint-disable @typescript-eslint/naming-convention */
        await snapshot.docs[0].ref.update({
            lastWebhookReceived: firestore_1.FieldValue.serverTimestamp(),
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
        /* eslint-enable @typescript-eslint/naming-convention */
        console.log(`[${ctx.trace_id}] Stamped lastWebhookReceived for plaidItemId=${plaid_item_id}`);
    },
};
