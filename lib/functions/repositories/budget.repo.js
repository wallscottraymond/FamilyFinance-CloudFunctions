...
exports.budget_repo = {
    ...
    async get_by_user_id(_ctx, user_id, options) {
        ...
        return snap.docs.map((doc) => map_to_entity(Object.assign(Object.assign({}, doc.data()), { id: doc.id })));
    },
    /**
     ...
     */
    async find_everything_else(_ctx, user_id, cadence) {
        var _a, _b, _c;
        const budgets = await this.get_by_user_id(_ctx, user_id);
        ...
        if (cadence) {
            return ((_b = (_a = ee.find((b) => (0, budgets_1.budget_cadence_to_instance)(b.period) === cadence)) !== null && _a !== void 0 ? _a : ee[0]) !== null && _b !== void 0 ? _b : null);
        }
        return (_c = ee[0]) !== null && _c !== void 0 ? _c : null;
    },
    /**
     ...
     */
    async set_tags(ctx, id, tag_ids, user_id) {
        const next = [...new Set(tag_ids)];
        /* eslint-disable @typescript-eslint/naming-convention */
        await doc_ref(id).update({ tags: next, updatedAt: firestore_1.Timestamp.now() });
        /* eslint-enable @typescript-eslint/naming-convention */
        (0, audit_1.record_audit_entry_async)({
            user_id,
            action: "update",
    ...
    async strip_tag(_ctx, user_id, tag_id) {
        ...
        for (const doc of snapshot.docs) {
            const current = (_b = (_a = doc.data()) === null || _a === void 0 ? void 0 : _a.tags) !== null && _b !== void 0 ? _b : [];
            const next = current.filter((t) => t !== tag_id);
            batch.update(doc.ref, { tags: next, updatedAt: firestore_1.Timestamp.now() });
            pending++;
            count++;
            if (pending === 450) {
                await batch.commit();
