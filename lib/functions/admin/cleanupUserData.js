...
exports.removeAllUserData = exports.removeAllUserTransactions = exports.removeAllUserInflows = exports.removeAllUserOutflows = exports.removeAllUserBudgets = exports.removeAllUserAccounts = void 0;
 ...
 */
exports.removeAllUserAccounts = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 300,
}, async (request) => {
    try {
        ...
        const { user, userData } = await (0, auth_1.authenticateRequest)(request, auth_1.UserRole.VIEWER);
        if (!user || !userData) {
        ...
        // Delete Plaid items
        try {
            const plaidItems = await (0, firestore_1.queryDocuments)('plaid_items', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const item of plaidItems) {
                await (0, firestore_1.deleteDocument)('plaid_items', item.id);
                totalDeleted++;
            }
        ...
        try {
            const plaidAccounts = await (0, firestore_1.queryDocuments)('plaid_accounts', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const account of plaidAccounts) {
                await (0, firestore_1.deleteDocument)('plaid_accounts', account.id);
                totalDeleted++;
            }
        ...
        try {
            const plaidTransactions = await (0, firestore_1.queryDocuments)('plaid_transactions', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const transaction of plaidTransactions) {
                await (0, firestore_1.deleteDocument)('plaid_transactions', transaction.id);
                totalDeleted++;
            }
        ...
        try {
            const accounts = await (0, firestore_1.queryDocuments)('accounts', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const account of accounts) {
                await (0, firestore_1.deleteDocument)('accounts', account.id);
                totalDeleted++;
            }
        ...
        throw new https_1.HttpsError('internal', `Account cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 * Remove all budgets and budget periods for a user
 */
exports.removeAllUserBudgets = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 300,
}, async (request) => {
    try {
        ...
        try {
            ...
            for (const budget of budgets) {
                await (0, firestore_1.deleteDocument)('budgets', budget.id);
                totalDeleted++;
            }
        ...
        try {
            const budgetPeriods = await (0, firestore_1.queryDocuments)('budget_periods', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const period of budgetPeriods) {
                await (0, firestore_1.deleteDocument)('budget_periods', period.id);
                totalDeleted++;
            }
        ...
        throw new https_1.HttpsError('internal', `Budget cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 * Remove all outflow transactions for a user
 */
exports.removeAllUserOutflows = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 300,
}, async (request) => {
    try {
        ...
        try {
            ...
            const outflows = await (0, firestore_1.queryDocuments)('outflows', {
            ...
            for (const outflow of outflows) {
                ...
                await (0, firestore_1.deleteDocument)('outflows', outflow.id);
                totalDeleted++;
            }
        ...
        try {
            ...
            const outflowPeriods = await (0, firestore_1.queryDocuments)('outflow_periods', {
                where: [{ field: 'ownerId', operator: '==', value: user.uid }]
            });
            ...
            for (const period of outflowPeriods) {
                ...
                await (0, firestore_1.deleteDocument)('outflow_periods', period.id);
                totalDeleted++;
            }
        ...
        throw new https_1.HttpsError('internal', `Outflow cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 * Remove all inflow transactions for a user
 */
exports.removeAllUserInflows = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 300,
}, async (request) => {
    try {
        ...
        // Delete inflows
        try {
            const inflows = await (0, firestore_1.queryDocuments)('inflows', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const inflow of inflows) {
                await (0, firestore_1.deleteDocument)('inflows', inflow.id);
                totalDeleted++;
            }
        ...
        try {
            const inflowPeriods = await (0, firestore_1.queryDocuments)('inflow_periods', {
                where: [{ field: 'userId', operator: '==', value: user.uid }]
            });
            for (const period of inflowPeriods) {
                await (0, firestore_1.deleteDocument)('inflow_periods', period.id);
                totalDeleted++;
            }
        ...
        throw new https_1.HttpsError('internal', `Inflow cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 * Remove all transactions for a user (including manual and Plaid transactions)
 */
exports.removeAllUserTransactions = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 300,
}, async (request) => {
    try {
                ...
                while (hasMore) {
                    ...
                    for (const transaction of transactions) {
                        ...
                        if (!deletedIds.has(transaction.id)) {
                            await (0, firestore_1.deleteDocument)('transactions', transaction.id);
        ...
        throw new https_1.HttpsError('internal', `Transaction cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 * Nuclear option: Remove ALL user data across all collections
 */
exports.removeAllUserData = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '1GiB',
    ...
    try {
        ...
        for (const collection of collections) {
            try {
                ...
                for (const field of queryFields) {
                    ...
                    let batchCount = 0;
                    while (hasMore) {
                        const items = await (0, firestore_1.queryDocuments)(collection, {
                            where: [{ field, operator: '==', value: user.uid }],
                            limit: 100
                        });
                        ...
                        for (const item of items) {
                            ...
                            if (!deletedIds.has(item.id)) {
                                ...
                                await (0, firestore_1.deleteDocument)(collection, item.id);
        ...
        throw new https_1.HttpsError('internal', `Complete cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
//# sourceMappingURL=cleanupUserData.js.map