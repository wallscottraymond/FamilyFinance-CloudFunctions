...
async function resolve_period_derivation_deps(ctx, user_id, view_cadence, window_start_ms, window_end_ms) {
    ...
    const [overlapping, budget_entities, monthly_period_docs, outflows, inflows, all_goals] = await Promise.all([
        repositories_1.source_period_repo.get_overlapping(ctx, firestore_1.Timestamp.fromMillis(window_start_ms), firestore_1.Timestamp.fromMillis(window_end_ms)),
        repositories_1.budget_repo.get_by_user_id(ctx, user_id),
        budget_period_repo_1.budget_period_repo.get_by_user_and_type(ctx, user_id, "monthly"),
        repositories_1.outflow_repo.get_by_user_id(ctx, user_id),
        repositories_1.inflow_repo.get_by_user_id(ctx, user_id),
        goal_repo_1.goal_repo.get_by_user(ctx, user_id),
    ]);
    ...
    for (const b of budget_entities) {
        ...
        const home_period_start_ms = (_b = overlapping.find((p) => p.period_type === to_cadence(b.period) &&
            raw_budget_start_ms >= p.start_date.toMillis() &&
            raw_budget_start_ms <= p.end_date.toMillis())) === null || _b === void 0 ? void 0 : _b.start_date.toMillis();
        const active_start_ms = is_ee ? 0 : home_period_start_ms !== null && home_period_start_ms !== void 0 ? home_period_start_ms : raw_budget_start_ms;
        ...
        // generation cascade.
        const materialized = (_c = monthly_by_budget.get(b.id)) !== null && _c !== void 0 ? _c : [];
    ...
    const { internal_ids, internal_plaid_ids } = (0, on_read_matching_1.detect_internal_transfers_from_txns)(txns);
    ...
    for (const o of outflows) {
        ...
        recurring.push({
            ...
            schedule: {
                ...
                average_amount: (_d = o.expected_amount_override) !== null && _d !== void 0 ? _d : o.average_amount,
        ...
        outflow_stream_items.push({ id: o.id, transaction_ids: (_e = o.transaction_ids) !== null && _e !== void 0 ? _e : [] });
    }
    ...
    for (const i of inflows) {
        ...
        recurring.push({
            ...
            schedule: {
                ...
                // User override ("this + future") wins over Plaid's average when set.
                average_amount: (_f = i.expected_amount_override) !== null && _f !== void 0 ? _f : i.average_amount,
        ...
        inflow_stream_items.push({ id: i.id, transaction_ids: (_g = i.transaction_ids) !== null && _g !== void 0 ? _g : [] });
    }
    ...
    const outflow_tx_to_id = (0, stream_membership_1.build_stream_membership_map)(outflow_stream_items);
    const inflow_tx_to_id = (0, stream_membership_1.build_stream_membership_map)(inflow_stream_items);
    ...
    for (const { id, data } of txns) {
        ...
        const plaid_txn_id = (_h = data.transactionId) !== null && _h !== void 0 ? _h : null;
        ...
        let income_amount = 0;
        const raw = (_j = data.splits) !== null && _j !== void 0 ? _j : [];
        for (const s of raw) {
            const outflow_id = (_k = s.outflowId) !== null && _k !== void 0 ? _k : null;
            const inflow_id = (_l = s.inflowId) !== null && _l !== void 0 ? _l : null;
            const amount = (_m = s.amount) !== null && _m !== void 0 ? _m : 0;
            const split_id = (_p = (_o = s.splitId) !== null && _o !== void 0 ? _o : s.id) !== null && _p !== void 0 ? _p : null;
            ...
            splits_for_match.push((0, on_read_matching_1.map_raw_split_to_on_read_match)(s, {
                txn_date_ms,
                is_pending,
        ...
        if (linked_outflow_id && payments_by_id.has(linked_outflow_id)) {
            const any_split_outflow_linked = raw.some((s) => { var _a; return ((_a = s.outflowId) !== null && _a !== void 0 ? _a : null) !== null; });
        ...
        const txn_cat = (_s = (_q = data.plaidDetailedCategory) !== null && _q !== void 0 ? _q : (_r = raw[0]) === null || _r === void 0 ? void 0 : _r.plaidDetailedCategory) !== null && _s !== void 0 ? _s : "";
        const any_split_inflow_linked = raw.some((s) => { var _a; return ((_a = s.inflowId) !== null && _a !== void 0 ? _a : null) !== null; });
        if (txn_cat.startsWith("INCOME") &&
            !linked_inflow_id &&
    ...
    for (const r of recurring) {
        r.payments = (_t = payments_by_id.get(r.id)) !== null && _t !== void 0 ? _t : [];
    }
    ...
    if (inflow_tx_ids.length > 0) {
        const history_by_inflow = new Map();
        const hist_docs = await transaction_repo_1.transaction_repo.get_by_plaid_transaction_ids(ctx, user_id, inflow_tx_ids);
        for (const d of hist_docs) {
            const inflow_id = inflow_tx_to_id.get(d.transactionId);
            if (!inflow_id)
                continue;
            const splits = (_u = d.splits) !== null && _u !== void 0 ? _u : [];
            const amount = Math.abs(splits.reduce((s, sp) => { var _a; return s + ((_a = sp.amount) !== null && _a !== void 0 ? _a : 0); }, 0) || d.amount || 0);
            const date_ms = d.transactionDate.toMillis();
            const list = (_v = history_by_inflow.get(inflow_id)) !== null && _v !== void 0 ? _v : [];
            list.push({ date_ms, amount });
            history_by_inflow.set(inflow_id, list);
        }
        for (const r of recurring) {
            if (r.kind === "inflow")
                r.payment_history = (_w = history_by_inflow.get(r.id)) !== null && _w !== void 0 ? _w : [];
        }
    }
