...
exports.resolve_account_removal_dependencies = resolve_account_removal_dependencies;
exports.resolve_account_balance_update_dependencies = resolve_account_balance_update_dependencies;
 ...
 */
async function resolve_account_removal_dependencies(ctx, account_id, user_id) {
    const span = (0, observability_1.create_span)(ctx, "resolver", "resolve_account_removal_dependencies");
    ...
    const account = await account_repo_1.account_repo.get_by_id(ctx, account_id, { include_deleted: true });
    if (!account) {
    ...
    const transaction_count = await transaction_repo_1.transaction_repo.count_by_account_id(ctx, plaid_account_id, user_id);
    // Get a sample of transaction IDs for reference (full cascade will query again)
    const transaction_ids = await transaction_repo_1.transaction_repo.get_ids_by_account_id(ctx, plaid_account_id, user_id, 100 // Limit to 100 for resolver - cascade job will handle all
    );
    transaction_ids.forEach(id => affected_entities.push(`transaction:${id}`));
    // 3. Get recurring outflows for this account
    const outflows = await outflow_repo_1.outflow_repo.get_by_account_id(ctx, plaid_account_id);
    ...
    const inflows = await inflow_repo_1.inflow_repo.get_by_account_id(ctx, plaid_account_id);
    ...
    const all_item_accounts = await account_repo_1.account_repo.get_by_item_id(ctx, item_id);
    const other_active_accounts = all_item_accounts.filter(a => a.id !== account_id && a.is_active);
    ...
    console.log(`[${ctx.trace_id}] resolve_account_removal_dependencies: ` +
 ...
 */
async function resolve_account_balance_update_dependencies(ctx, account_id, user_id) {
    const span = (0, observability_1.create_span)(ctx, "resolver", "resolve_account_balance_update_dependencies");
    (0, observability_1.log_operation_start)(span, user_id);
