...
async function runUpdateInflowPeriods(db, inflowId, inflowBefore, inflowAfter) {
    ...
    try {
        ...
        const effectiveAmountBefore = (_a = inflowBefore.expectedAmountOverride) !== null && _a !== void 0 ? _a : inflowBefore.averageAmount;
        const effectiveAmount = (_b = inflowAfter.expectedAmountOverride) !== null && _b !== void 0 ? _b : inflowAfter.averageAmount;
        if (effectiveAmountBefore !== effectiveAmount) {
            changedFields.push('averageAmount');
            console.log(`[runUpdateInflowPeriods] effective expected amount changed: ${effectiveAmountBefore} → ${effectiveAmount} (override=${(_c = inflowAfter.expectedAmountOverride) !== null && _c !== void 0 ? _c : 'none'})`);
        }
        ...
        const beforePredicted = (_f = (_e = (_d = inflowBefore.predictedNextDate) === null || _d === void 0 ? void 0 : _d.toMillis) === null || _e === void 0 ? void 0 : _e.call(_d)) !== null && _f !== void 0 ? _f : null;
        const afterPredicted = (_j = (_h = (_g = inflowAfter.predictedNextDate) === null || _g === void 0 ? void 0 : _g.toMillis) === null || _h === void 0 ? void 0 : _h.call(_g)) !== null && _j !== void 0 ? _j : null;
        ...
        // EXPECTED-amount + name changes propagate to ALL periods — INCLUDING received ones.
        // The EXPECTED income is independent of receipt; only the ACTUAL received amount
        // (totalAmountPaid, occurrenceAmounts) is tied to the real transaction and is preserved.
        // Previously amount changes were gated to unreceived periods, so editing income whose current
        // period was already received silently did nothing while the name changed. (See outflow twin.)
        const periodsToUpdate = allPeriods;
        result.periodsSkipped = 0;
        console.log(`[runUpdateInflowPeriods] Periods to update: ${periodsToUpdate.length} (all; ${receivedCount} received get EXPECTED-only updates, actual received amounts preserved)`);
        ...
        if (changedFields.includes('predictedNextDate') || changedFields.includes('averageAmount')) {
            ...
            const prediction = (0, predictNextPayment_1.predictNextPayment)(Object.assign(Object.assign({}, inflowAfter), { averageAmount: effectiveAmount }));
            if (prediction) {
                predictionData = {
        ...
        for (let i = 0; i < periodsToUpdate.length; i += batchSize) {
            ...
            for (const periodDoc of batchPeriods) {
                ...
                // Handle averageAmount change on ALL periods (received included) — EXPECTED only.
                // Uses the EFFECTIVE amount (override ?? average). Actual received (totalAmountPaid,
                // received occurrenceAmounts) is preserved below.
                if (changedFields.includes('averageAmount')) {
                    ...
                    // ACTUAL received (totalAmountPaid) preserved; only derived "unpaid" recomputed (clamped ≥0).
                    updates.totalAmountUnpaid = Math.max(0, updates.totalAmountDue - (period.totalAmountPaid || 0));
                ...
                if (changedFields.includes('isRegularSalary')) {
                    updates.isRegularSalary = (_k = inflowAfter.isRegularSalary) !== null && _k !== void 0 ? _k : false;
                ...
                if (changedFields.includes('isActive') && !isReceived) {
                    updates.isActive = (_l = inflowAfter.isActive) !== null && _l !== void 0 ? _l : true;
