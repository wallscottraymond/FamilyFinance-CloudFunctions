...
exports.generate_expected_occurrences_in_window = generate_expected_occurrences_in_window;
...
const frequency_1 = require("../recurring/frequency");
/**
...
function get_cycle_days(frequency) {
    switch ((0, frequency_1.normalize_frequency)(frequency)) {
        case "WEEKLY":
            return 7;
        case "BIWEEKLY":
            return 14;
...
function adjust_for_month_end(current_date, reference_date, frequency) {
    const freq = (0, frequency_1.normalize_frequency)(frequency);
 ...
 */
function semimonthly_days(reference) {
 ...
 */
function semimonthly_due_dates(reference, start, end) {
...
function calculate_occurrences_in_period(outflow, source_period, cycle_info) {
    ...
    if ((0, frequency_1.normalize_frequency)(frequency) === "SEMIMONTHLY") {
        ...
        for (const dt of semimonthly_due_dates(reference_date, period_start, period_end)) {
            occurrence_due_dates.push(firestore_1.Timestamp.fromDate(dt));
        }
        // Next expected = the first semi-monthly day after the period (scan ~45 days out).
        const after = semimonthly_due_dates(reference_date, new Date(period_end.getTime() + 1), new Date(period_end.getTime() + 45 * 24 * 60 * 60 * 1000));
        next_expected_date = firestore_1.Timestamp.fromDate((_a = after[0]) !== null && _a !== void 0 ? _a : new Date(period_end.getTime() + 1));
    }
    else {
        ...
        next_expected_date = firestore_1.Timestamp.fromDate(adjust_for_month_end(current_date, reference_date, frequency));
    }
 ...
 */
function generate_expected_occurrences_in_window(schedule, window_start_ms, window_end_ms) {
    ...
    const window = {
        ...
        period_id: "window",
        type: "custom",
        start_date: firestore_1.Timestamp.fromMillis(window_start_ms),
        end_date: firestore_1.Timestamp.fromMillis(window_end_ms),
    };
