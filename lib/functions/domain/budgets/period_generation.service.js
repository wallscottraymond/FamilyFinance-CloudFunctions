...
exports.EE_HISTORY_BACKDATE_MONTHS = void 0;
exports.budget_cadence_to_instance = budget_cadence_to_instance;
exports.compute_period_generation_end = compute_period_generation_end;
exports.build_self_provision_budget_created_payload = build_self_provision_budget_created_payload;
exports.compute_ee_coverage_start = compute_ee_coverage_start;
exports.compute_budget_periods = compute_budget_periods;
exports.compute_prime_contributions = compute_prime_contributions;
exports.compute_reallocated_periods = compute_reallocated_periods;
exports.count_days_inclusive = count_days_inclusive;
exports.days_in_month = days_in_month;
exports.compute_period_allocation = compute_period_allocation;
exports.compute_daily_rate = compute_daily_rate;
 ...
 */
function budget_cadence_to_instance(period) {
 ...
 * PURE FUNCTION.
 */
function compute_period_generation_end(start, is_ongoing, budget_end_date) {
 ...
 * PURE FUNCTION.
 */
function build_self_provision_budget_created_payload(args) {
    var _a;
    const generation_end = compute_period_generation_end(args.start, args.is_ongoing, args.budget_end_date);
    const window_start = (_a = args.coverage_start) !== null && _a !== void 0 ? _a : args.start;
    return {
 ...
 */
exports.EE_HISTORY_BACKDATE_MONTHS = 24;
/**
 ...
 */
function compute_ee_coverage_start(reference) {
    const start = new Date(reference.getTime());
    start.setUTCMonth(start.getUTCMonth() - exports.EE_HISTORY_BACKDATE_MONTHS);
    return start;
}
/**
 ...
 */
function compute_budget_periods(input) {
    var _a, _b;
    if (!input.budget_id) {
    ...
    const make_base = (source, allocated, daily_rate) => ({
    ...
    const prime_entities = [];
    const primes_for_contrib = [];
    for (const source of input.source_periods) {
        ...
        const daily_rate = (_a = rate_result.entity) !== null && _a !== void 0 ? _a : 0;
    ...
    for (const source of input.source_periods) {
        ...
        const start = source.start_date.toDate();
        const end = source.end_date.toDate();
        const overlapping = primes_for_contrib.filter((p) => p.start <= end && p.end >= start);
        const { total_amount, breakdown } = compute_prime_contributions(start, end, overlapping);
        ...
        const daily_rate = (_b = rate_result.entity) !== null && _b !== void 0 ? _b : 0;
        const entity = make_base(source, allocated, daily_rate);
 ...
 * PURE FUNCTION.
 */
function compute_prime_contributions(target_start, target_end, overlapping_primes) {
    var _a, _b;
    ...
    let total_amount = 0;
    let cursor = Date.UTC(target_start.getUTCFullYear(), target_start.getUTCMonth(), target_start.getUTCDate());
    const end_utc = Date.UTC(target_end.getUTCFullYear(), target_end.getUTCMonth(), target_end.getUTCDate());
    ...
    for (const c of contributions.values()) {
        breakdown.push({
            ...
            overlap_start: firestore_1.Timestamp.fromDate((_a = c.overlap_start) !== null && _a !== void 0 ? _a : c.prime.start),
            overlap_end: firestore_1.Timestamp.fromDate((_b = c.overlap_end) !== null && _b !== void 0 ? _b : c.prime.end),
        });
    }
...
function utc_day_keys(start, end) {
    const keys = [];
...
function compute_reallocated_periods(input) {
    ...
    for (const period of input.periods) {
        ...
        if (!is_historical(period)) {
            ...
            effective_rate = (_a = rate_result.entity) !== null && _a !== void 0 ? _a : 0;
            updates.push({
                id: period.id,
                allocated_amount: allocated,
 ...
 * PURE FUNCTION.
 */
function count_days_inclusive(start, end) {
 ...
 * PURE FUNCTION.
 */
function compute_period_allocation(input) {
    if (input.budget_amount < 0) {
        return { validation_errors: ["budget_amount cannot be negative"] };
    }
    ...
    switch (input.budget_period_type) {
        case "monthly":
            allocation = allocate_monthly_to_target(input.budget_amount, input.target_start, input.target_end);
            break;
        case "weekly": {
 ...
 * PURE helper.
 */
function allocate_monthly_to_target(monthly_amount, target_start, target_end) {
    let total = 0;
    const cursor = new Date(Date.UTC(target_start.getUTCFullYear(), target_start.getUTCMonth(), target_start.getUTCDate()));
    const end_utc = Date.UTC(target_end.getUTCFullYear(), target_end.getUTCMonth(), target_end.getUTCDate());
 ...
 * PURE FUNCTION.
 */
function compute_daily_rate(allocated_amount, period_days, is_prime) {
    if (period_days <= 0) {
...
//# sourceMappingURL=period_generation.service.js.map