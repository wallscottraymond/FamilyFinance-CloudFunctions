...
exports.validate_outflows_for_sync = validate_outflows_for_sync;
exports.detect_duplicate_outflows = detect_duplicate_outflows;
exports.compute_outflow_merge_suggestions = compute_outflow_merge_suggestions;
exports.filter_outflows_needing_review = filter_outflows_needing_review;
exports.compute_outflow_status = compute_outflow_status;
exports.categorize_outflows_by_type = categorize_outflows_by_type;
exports.compute_monthly_outflow_total = compute_monthly_outflow_total;
 ...
 */
function validate_outflows_for_sync(outflows) {
    const valid_outflows = [];
    ...
    for (const outflow of outflows) {
        ...
        if (errors.length === 0) {
            ...
            validation_errors.push(`Outflow ${outflow.id}: ${errors.join(", ")}`);
        }
    }
    if (validation_errors.length > 0) {
        return (0, types_1.partial_success)(valid_outflows, validation_errors);
    }
    return (0, types_1.success_many)(valid_outflows);
}
/**
 ...
 * PURE FUNCTION.
 */
function validate_single_outflow(outflow) {
    const errors = [];
    // Required fields
    if (!outflow.id) {
    ...
    if (!valid_frequencies.includes(outflow.frequency)) {
 ...
 */
function detect_duplicate_outflows(new_outflows, existing_outflows) {
    const result = {
        new_outflows: [],
    ...
    for (const new_outflow of new_outflows) {
        ...
        const exact_match = existing_outflows.find((e) => e.plaid_stream_id === new_outflow.plaid_stream_id);
        if (exact_match) {
 ...
 * PURE FUNCTION.
 */
function find_similar_outflow(new_outflow, existing_outflows) {
 ...
 * PURE FUNCTION.
 */
function calculate_outflow_match(new_outflow, existing) {
    const reasons = [];
    let score = 0;
    // Name matching (merchant)
    const name_match = names_match(new_outflow.merchant_name || new_outflow.description, existing.merchant_name || existing.description);
    if (name_match === "exact") {
        score += 3;
 ...
 */
function compute_outflow_merge_suggestions(plaid_outflows, manual_outflows) {
    const suggestions = [];
    // Only consider active manual outflows
    const active_manual = manual_outflows.filter((m) => m.is_active && m.source === "manual");
 ...
 */
function filter_outflows_needing_review(outflows) {
 ...
 */
function compute_outflow_status(has_similar_manual, plaid_confidence) {
 ...
 */
function categorize_outflows_by_type(outflows) {
 ...
 */
function compute_monthly_outflow_total(outflows) {
    let total = 0;
    for (const outflow of outflows) {
        if (!outflow.is_active)
            continue;
        const monthly_amount = normalize_to_monthly(outflow.average_amount, outflow.frequency);
 ...
 * PURE FUNCTION.
 */
function normalize_to_monthly(amount, frequency) {
    switch (frequency) {
        case "weekly":
...
//# sourceMappingURL=outflow.service.js.map