...
exports.validate_update_link_token_request = validate_update_link_token_request;
exports.check_item_status_for_relink = check_item_status_for_relink;
exports.should_show_help_message = should_show_help_message;
exports.get_update_link_token_error_message = get_update_link_token_error_message;
 ...
 */
function validate_update_link_token_request(input) {
    const errors = [];
    ...
    if (input.recent_relink_attempts >= update_link_token_types_1.MAX_RELINK_ATTEMPTS_BEFORE_HELP) {
 ...
 */
function check_item_status_for_relink(status) {
    ...
    if (update_link_token_types_1.STATUSES_REQUIRING_REAUTH.includes(status)) {
        return { relink_disabled: false, disabled_reason: null };
    }
    // Check for special status messages
    if (update_link_token_types_1.STATUS_ERROR_MESSAGES[status]) {
        return {
            relink_disabled: true,
            disabled_reason: update_link_token_types_1.STATUS_ERROR_MESSAGES[status],
        };
    }
 ...
 */
function should_show_help_message(recent_attempts) {
    if (recent_attempts >= update_link_token_types_1.MAX_RELINK_ATTEMPTS_BEFORE_HELP) {
        return {
            show_help: true,
            message: "Having trouble reconnecting? Try visiting your bank's website to verify your login credentials, " +
 ...
 */
function get_update_link_token_error_message(error_code) {
...
//# sourceMappingURL=update_link_token.service.js.map