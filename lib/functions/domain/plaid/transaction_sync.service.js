...
exports.validate_transactions_for_sync = validate_transactions_for_sync;
exports.detect_material_changes = detect_material_changes;
exports.calculate_proportional_splits = calculate_proportional_splits;
exports.merge_pending_to_posted = merge_pending_to_posted;
exports.aggregate_transaction_sync_results = aggregate_transaction_sync_results;
exports.should_continue_sync = should_continue_sync;
exports.normalize_amount = normalize_amount;
exports.determine_transaction_type = determine_transaction_type;
 ...
 */
function validate_transactions_for_sync(transactions) {
 ...
 * PURE FUNCTION.
 */
function validate_single_transaction(txn) {
    const errors = [];
...
function detect_material_changes(existing, new_data) {
    ...
    // Category change
    if (new_data.plaid_primary_category !== existing.plaid_primary_category ||
 ...
 */
function calculate_proportional_splits(splits, old_amount, new_amount) {
    if (old_amount === 0) {
        // Avoid division by zero - distribute evenly
        const per_split = new_amount / splits.length;
        return splits.map(s => (Object.assign(Object.assign({}, s), { amount: per_split })));
    }
    const ratio = new_amount / old_amount;
    return splits.map(s => (Object.assign(Object.assign({}, s), { amount: Math.round(s.amount * ratio * 100) / 100 })));
}
/**
 ...
 */
function merge_pending_to_posted(posted_transaction, migration) {
    const pending = migration.pending_transaction;
    // Calculate adjusted splits if amount changed
    let migrated_splits;
    if (migration.amount_changed) {
        migrated_splits = calculate_proportional_splits(pending.splits, migration.old_amount, migration.new_amount);
    }
    else {
        migrated_splits = pending.splits;
    }
    // Convert migrated splits to persistence format
    const final_splits = migrated_splits.map((pending_split, index) => {
        // Get corresponding split from posted transaction for Plaid categories
        const posted_split = posted_transaction.splits[index] || posted_transaction.splits[0];
        return {
            ...
            monthly_period_id: (posted_split === null || posted_split === void 0 ? void 0 : posted_split.monthly_period_id) || null, // Use posted periods
            weekly_period_id: (posted_split === null || posted_split === void 0 ? void 0 : posted_split.weekly_period_id) || null,
            bi_weekly_period_id: (posted_split === null || posted_split === void 0 ? void 0 : posted_split.bi_weekly_period_id) || null,
            plaid_primary_category: (posted_split === null || posted_split === void 0 ? void 0 : posted_split.plaid_primary_category) || posted_transaction.plaid_primary_category,
            plaid_detailed_category: (posted_split === null || posted_split === void 0 ? void 0 : posted_split.plaid_detailed_category) || posted_transaction.plaid_detailed_category,
    ...
    return Object.assign(Object.assign({}, posted_transaction), { 
        // Preserve user's internal categories if set
        internal_primary_category: pending.internal_primary_category ||
            posted_transaction.internal_primary_category, internal_detailed_category: pending.internal_detailed_category ||
            posted_transaction.internal_detailed_category, 
        // Use migrated splits
        splits: final_splits, 
        // Clear pending status (now posted)
        is_pending: false, pending_transaction_id: migration.pending_plaid_transaction_id });
}
// ============================================================================
 ...
 * PURE FUNCTION.
 */
function aggregate_transaction_sync_results(added_count, modified_count, removed_count, pending_migrated_count, has_more, next_cursor, errors) {
 ...
 */
function should_continue_sync(errors, max_errors = 10) {
 ...
 */
function normalize_amount(plaid_amount) {
 ...
 * PURE FUNCTION.
 */
function determine_transaction_type(plaid_amount) {
    if (plaid_amount < 0) {
        return "income";
    }
    return "expense";
}
//# sourceMappingURL=transaction_sync.service.js.map