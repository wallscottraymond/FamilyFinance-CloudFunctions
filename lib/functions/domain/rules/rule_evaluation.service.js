...
exports.evaluate_rules = evaluate_rules;
exports.evaluate_group = evaluate_group;
 ...
 */
function evaluate_rules(txn, rules) {
    var _a, _b, _c;
    const intents = { applied_rule_ids: [] };
    ...
    const running = Object.assign(Object.assign({}, txn), { tags: [...((_a = txn.tags) !== null && _a !== void 0 ? _a : [])] });
    // Ascending priority = top of the Rule Book first; later (lower in the list) overrides earlier.
    const ordered = [...rules].sort((a, b) => a.priority - b.priority);
    for (const rule of ordered) {
        if (!rule.is_active)
            continue;
        if (!evaluate_group(running, rule.conditions))
            continue;
        ...
        if (rule.actions.add_tag && rule.actions.add_tag.length > 0) {
            const seen = new Set((_b = running.tags) !== null && _b !== void 0 ? _b : []);
            for (const id of rule.actions.add_tag)
                if (!seen.has(id))
                    ((_c = running.tags) !== null && _c !== void 0 ? _c : (running.tags = [])).push(id);
        }
    }
    return intents;
}
/** Merge one matched rule's actions into the running intents (scalars: last-wins; booleans: OR). */
function accumulate_actions(actions, intents) {
    var _a, _b;
    if (actions.assign_budget_id !== undefined)
        intents.assign_budget_id = actions.assign_budget_id;
    if (actions.assign_category !== undefined)
        intents.assign_category = actions.assign_category;
    if (actions.split !== undefined)
        intents.split = actions.split;
    if (actions.make_recurring !== undefined)
        intents.make_recurring = actions.make_recurring;
    if (actions.ignore)
        intents.ignore = true;
    if (actions.mark_refund)
        intents.mark_refund = true;
    if (actions.mark_income)
        intents.mark_income = true;
    if (actions.add_tag && actions.add_tag.length > 0) {
        const seen = new Set((_a = intents.add_tag) !== null && _a !== void 0 ? _a : []);
        intents.add_tag = (_b = intents.add_tag) !== null && _b !== void 0 ? _b : [];
        for (const id of actions.add_tag)
            if (!seen.has(id))
                intents.add_tag.push(id);
    }
    if (actions.require_note)
        intents.require_note = true;
    if (actions.require_review)
        intents.require_review = true;
}
/** Evaluate a condition group with short-circuit AND/OR + recursive nested groups. */
function evaluate_group(txn, group) {
    var _a;
    ...
    if (group.conditions.length === 0 && nested.length === 0)
        return false;
    if (group.op === "AND") {
        for (const c of group.conditions)
            if (!evaluate_condition(txn, c))
                return false;
        for (const g of nested)
            if (!evaluate_group(txn, g))
                return false;
        return true;
    }
    // OR
...
function evaluate_condition(txn, c) {
    var _a, _b, _c;
    switch (c.variable) {
        case "merchant":
            // Match against BOTH the cleaned merchant name AND the raw description, case-insensitive.
            return (match_string((_a = txn.merchant_name) !== null && _a !== void 0 ? _a : "", c) || match_string((_b = txn.name) !== null && _b !== void 0 ? _b : "", c));
        case "category":
        ...
        case "tag":
            return match_tag((_c = txn.tags) !== null && _c !== void 0 ? _c : [], c);
        default:
            return assert_never_variable(c.variable);
    }
}
/** Tag variable: `has` — true when the running tag set contains the given tag id. */
function match_tag(tags, c) {
    var _a;
    const target = String((_a = c.value) !== null && _a !== void 0 ? _a : "").trim();
    if (target.length === 0)
        return false;
    switch (c.operator) {
        case "has":
...
function effective_category(txn) {
    var _a, _b;
    return (_b = (_a = txn.internal_primary_category) !== null && _a !== void 0 ? _a : txn.plaid_primary_category) !== null && _b !== void 0 ? _b : "";
}
/** String variables (merchant/category/account): `contains`/`equals`, case-insensitive. */
function match_string(field_value, c) {
    var _a;
    const target = String((_a = c.value) !== null && _a !== void 0 ? _a : "").trim().toLowerCase();
    if (target.length === 0)
        return false;
...
function match_amount(abs_amount, c) {
...
function match_date(txn_date, c) {
...
function assert_never_variable(_v) {
    return false;
}
//# sourceMappingURL=rule_evaluation.service.js.map