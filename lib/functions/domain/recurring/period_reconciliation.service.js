...
exports.compute_countable_split_net = compute_countable_split_net;
...
function compute_period_reconciliation(period, linked_splits) {
    ...
    // and a payment would never reconcile. If a payment landed here (posted OR
    // pending — we now count pending), treat the period as a single occurrence
    // (presence-based) so it still marks paid until regenerated with real data.
    if (due.length === 0) {
        const pay_dates = linked_splits
            .filter((s) => s.amount > AMOUNT_EPSILON)
            .map((s) => s.date_ms);
        if (pay_dates.length > 0) {
            due = [Math.min(...pay_dates)];
        }
    }
    const occurrences_expected = due.length;
    const expected_amount = occurrences_expected * period.amount_per_occurrence;
    // Net each payment into its NEAREST occurrence by date. We COUNT PENDING toward
    // paid (per the count-but-flag decision), but remember per-occurrence how much of
    // its net came from pending so we can flag an occurrence that's paid ONLY by a
    // still-pending payment. Posted payments take precedence for the shown txn id.
    const occ_sum = new Array(due.length).fill(0); // combined (posted+pending)
    const occ_pending_sum = new Array(due.length).fill(0);
    const occ_txn = new Array(due.length).fill(null);
    const occ_pending_txn = new Array(due.length).fill(null);
    for (const s of linked_splits) {
        ...
        if (s.is_pending) {
            occ_pending_sum[best] += s.amount;
            if (s.amount > AMOUNT_EPSILON && occ_pending_txn[best] === null) {
                occ_pending_txn[best] = s.transaction_id;
            }
        }
        else if (s.amount > AMOUNT_EPSILON && occ_txn[best] === null) {
            occ_txn[best] = s.transaction_id; // posted wins the displayed id
        }
    }
    // An occurrence is paid when its net amount is positive (variable) or meets the
    // per-occurrence amount (fixed) — counting posted + pending. It's `pending` when
    // paid but its net came entirely from pending payments (no posted portion).
    const occurrences = due.map((d, i) => {
        var _a;
        const paid = period.is_variable_amount
            ? occ_sum[i] > AMOUNT_EPSILON
            : occ_sum[i] >= period.amount_per_occurrence - AMOUNT_EPSILON;
        const posted_portion = occ_sum[i] - occ_pending_sum[i];
        const pending = paid && posted_portion <= AMOUNT_EPSILON && occ_pending_sum[i] > AMOUNT_EPSILON;
        return {
            due_date_ms: d,
            paid,
            transaction_id: (_a = occ_txn[i]) !== null && _a !== void 0 ? _a : occ_pending_txn[i],
            amount: occ_txn[i] === null && occ_pending_txn[i] === null ? null : occ_sum[i],
            pending,
        };
    });
    const occurrences_paid = occurrences.filter((o) => o.paid).length;
    const has_pending = occurrences.some((o) => o.pending);
    // Status counts pending toward paid, so use the COMBINED amount.
    const status = compute_status({
        is_variable: period.is_variable_amount,
        occurrences_expected,
        occurrences_paid,
        matched_amount: matched_amount + pending_amount,
        expected_amount,
    });
    return {
 ...
 */
function compute_countable_split_net(splits, fallback_split_id) {
    var _a;
    let amount = 0;
    ...
    for (const s of splits) {
        if (s.isIgnored)
            continue;
        const magnitude = Math.abs(Number((_a = s.amount) !== null && _a !== void 0 ? _a : 0));
