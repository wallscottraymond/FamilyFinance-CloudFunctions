...
const createEverythingElseBudget_1 = require("../budgets/utils/createEverythingElseBudget");
/**
...
exports.onUserCreate = functions.region("us-central1").runWith({
    ...
    try {
        ...
        const userRole = isDevMode ? types_1.UserRole.ADMIN : types_1.UserRole.EDITOR;
        ...
        // Initialize user data (summaries and budgets) - AWAITED for proper initialization
        // Frontend waits for isInitialized flag before loading data
        ...
        try {
            ...
            const [summariesResult, budgetId] = await Promise.all([
                ...
                (0, preCreateUserPeriodSummaries_1.preCreateUserPeriodSummaries)(userRecord.uid)
                    .then(() => {
                ...
                (0, createEverythingElseBudget_1.createEverythingElseBudget)(db, userRecord.uid, detectedCurrency)
                    .then((id) => {
            ...
            await db.collection('users').doc(userRecord.uid).update({
                isInitialized: true,
                initializedAt: firestore_1.Timestamp.now()
            });
            ...
            // Still mark as initialized to prevent frontend from waiting forever
            // Individual features will handle missing data gracefully
            await db.collection('users').doc(userRecord.uid).update({
                isInitialized: true,
                initializedAt: firestore_1.Timestamp.now()
            }).catch(() => {
                console.error(`❌ Failed to set isInitialized flag for ${userRecord.uid}`);
            });
        }
