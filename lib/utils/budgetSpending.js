...
async function updateBudgetPeriodSpending(budgetId, spendingDelta, userId, transactionDate) {
    ...
    const budgetPeriods = await index_1.db.collection('budget_periods')
    ...
    matchingPeriods.forEach(periodDoc => {
        ...
        batch.update(periodDoc.ref, {
            spent: firestore_1.FieldValue.increment(spendingDelta),
            remaining: firestore_1.FieldValue.increment(-spendingDelta), // Decrease remaining by delta
...
async function recalculateBudgetSpendingOnCreate(budgetId, budget) {
    ...
    try {
        ...
        const transactionsSnapshot = await index_1.db.collection('transactions')
